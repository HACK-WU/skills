...
**通用加压项——数据守恒三问**（代码变更模式叠加在所选策略之上，变更涉及数据生产/转换/消费时必做）：
...
> 判定规则：任一问命中变化且消费方受影响 → 质疑成立（按影响面定风险等级）；变化但声称"所有消费方兼容"——要求拿出消费方清单作证，拿不出则质疑仍成立。报告文档模式下，若被质疑报告本身含影响面/数据流分析，同样用三问检验其分析是否遗漏了消费方。
...
| **光环质疑** | 搭车变更是否借主修复逃过了独立评审？审查报告对它的审查强度与主变更一致吗？ | 检查审查报告是否对搭车变更单独做过全局回扣与守恒三问，而非一句"顺手优化"带过 |

> 举证倒置：声称"顺手优化无副作用"须同时拿出两份证据——原写法非故意设计的证据 + 全局无影响的论证；缺任一份，质疑成立（看不懂的围栏不能拆），建议拆分为独立提交单独评审。审查报告对搭车变更未单独立案的，该遗漏本身作为成立质疑写入报告（风险至少 🟡 中）；结果作为常规质疑点写入质疑详情，🏷️ 类型标注为「搭车变更」。
...
8. **数据守恒优先（代码变更模式）**："代码正确但数据悄悄变少"是评审最容易放过的静默 Bug，凡变更触碰数据链路，守恒三问（流向/数量/内容）不得跳过
9. **全局回扣兜底**：逐点质疑全部通过不等于整体成立，每次质疑必须以全局回扣质疑（目标/位置/判据）收尾；被质疑方给不出全局图景时，该缺失本身作为成立质疑写入报告
10. **搭车变更零信任（代码变更模式）**："顺手优化"默认视为未授权、未证明——它可能拆掉了前人故意立的围栏；举证责任在改动方，拿不出「原写法非故意 + 全局无影响」双重证据的，质疑成立并建议拆分独立提交
