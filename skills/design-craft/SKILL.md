...
按 `expert-solution-workflow` 规则查询当前需求涉及的模块的相关经验、解决方案或记忆（如该模块的业务专家资产包；查询失败或不可用，忽略此步骤继续正常流程）：
...
**协作地图辅助——强关联关系查询**：建立交互对象清单时，可调用 `use_skill("relation-lookup")` 检索涉及模块的强关联关系（跨模块契约/业务耦合），作为协作地图中"模块间牵动关系"的补充依据，避免设计时遗漏已有代码的强耦合（查询失败或无可用记录，忽略继续）。
