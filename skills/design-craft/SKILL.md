...
- ❌ 一次性填充所有子文档等用户确认：上下文爆炸，用户难以逐项审查
- ❌ 自动评审循环超过 3 次仍继续修改：可能陷入死循环，应停止并交人工处理
- ❌ 跳过 challenger 二次质疑：design-review 通过后必须调用 challenger 深度审查
- ❌ challenger 发现问题后不询问用户直接修复：设计类问题需人工权衡，应提供修复/落盘/跳过选项
...
- ❌ 非功能性需求未显式标注默认假设
- ❌ 待定问题写"无"但方案中存在未决依赖
- ❌ 代码不写文件路径：设计文档中的代码变更必须标注具体路径
- ❌ API 接口无 Demo 返回示例：新增/修改接口必须附带 JSON 响应示例
- ❌ 接口未确定就先写实现：必须先确认接口契约再编写实现设计
