...
> 4. "重新评估触发条件"必须是**可观测判据**（阈值 / 依赖变化 / 技术栈支持，如"QPS>5000""依赖方支持 webhook"），写不出可观测判据就写「无」，禁止"性能不足时""业务变化时"这类不可判定的空话
> 5. 本表**不写 ki**：设计阶段决策仍在频繁变化，不入知识库。本表是"决策记忆"的数据源——模块实现落地后，可由 `expert-team` Step 8.6 或手动调 `ki-memory-write` 决策记忆策略收割入 ki
