...
- **{标准叫法：中文（英文）；无公认中文译名则直接用英文 / 缩写——不硬造}**：{一句话}——就是本课说的"{回指本课的说法}"；在哪遇到：{配置项 / 监控指标 / 报错关键字 / 官方文档章节}
...
> 🔴 标准叫法 / 配置项 / 报错原文须过事实核查闸门（优先查 Step 1.4 的 `web-index/`），**不得编造**；**中文译名同样不得硬造**——"翻得出来"≠"有人在用"，无公认译名时用英文 / 缩写（命名三态见「术语锚定规范」）；确无行业术语的纯概念知识点，标注"本知识点无行业术语"并省略本节。
...
- **OSR（Out-of-Sync Replicas）**：**无通用中文译名，直接用英文缩写、不硬造**——就是本课说的"落榜区"；在哪遇到：`kafka-topics.sh --describe` 输出中**在 `Replicas` 里但不在 `Isr` 里**的编号（两者相减即 OSR）
...
- **命名三态（中文译名以"真的有人在用"为准，不以"翻得出来"为准）**：① **有公认中文译名** → `中文（English）`（如 幂等（Idempotence））；② **无公认中文译名** → **直接用英文 / 缩写，绝不硬造中文**——实证：`OSR` / `Out-of-Sync Replicas` **没有通用中文译名**，硬翻成"失同步副本集"之类的说法，读者拿去沟通反而对不上号；③ 业界惯用缩写 → `缩写（全称）`（如 `ISR（In-Sync Replicas）`）。判断依据看官方中文文档 / 权威教材是否在用；**拿不准就不用中文，英文不会错**。（三态只决定"**叫什么**"——"人话解释"与"回指直觉层说法"照旧不可少：`OSR` 该解释还得解释，只是**不给它硬安一个中文名**）；反之**有中文译名时不要只甩英文**——中文读者靠译名读得快，别把"不硬造"执行成"一律用英文"
