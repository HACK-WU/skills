...
**定位**：Phase 1.3 落盘时在**产物仓库根**创建；后续按 SKILL.md「产物仓库卫生」的「检查方法」四步增量追加。**只追加不删规则**，不预先灌满未出现的类别。（清理临时文件另见 SKILL.md 同章节的「临时文件清理」小节，两者不是一回事）
...
| **追加后校验** | `git status --porcelain` 中 `*.md` / `assets/*.svg` / `实现/` 源码仍可见；或对关键产物跑 `git check-ignore -v {路径}` 应无命中 |
