...
**定位**：`应用实战/` 目录——每课一篇应用实战（第四幕 4.2 的正文落在这里，**课内只留入口块**），与课**一一对应**；`应用实战/INDEX.md` 是**一键跳转全部实战的固定入口**（活文档，随课程进度更新，与 `02-课程目录.md` 同节奏）。
...
- **无实战的课**（环境准备课 / 直述模式课）不写 4.2（无入口块、无实战文件），在 INDEX 中标"无实战"；速览模式**不独立成篇**——轻量版留在 `overview.md` 内
