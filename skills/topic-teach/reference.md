...
速览模式 `overview.md` 为其浓缩版，可省略课后小测，但须含"速览大纲"与"延伸学习提示词"，见 SKILL.md 速览模式；**「一句话本质」与「一眼全局图」不可省略**——速览是单篇讲透，入口可读性反而更关键。**注意图路径不同**：速览 `overview.md` 在 `{topic-slug}/` 根，用 `./assets/xxx.svg`；单课在 `lessons/` 子目录，才用 `../assets/xxx.svg`——**勿照抄单课模板的 `../assets/`**（层级规则见 reference.md「SVG 产出形式」）。
...
> **与第三幕「一眼全局图」的分工（勿混）**：一眼全局图在**课首**、**问题视角**、**零术语**，给**没学过的人**看（回答"为什么需要它、靠什么思路"）；一图总结在**课末**、**知识视角**，浓缩本课知识点与彼此关系，给**刚学完的人**复习用（回答"本课讲了什么"）。两者不可互相替代，也不得内容雷同。
