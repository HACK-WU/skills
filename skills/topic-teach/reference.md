...
- **{标准叫法（英文）}**：{一句话}——就是本课说的"{回指本课的说法}"；在哪遇到：{配置项 / 监控指标 / 报错关键字 / 官方文档章节}
...
> 🔴 标准叫法 / 配置项 / 报错原文须过事实核查闸门（优先查 Step 1.4 的 `web-index/`），**不得编造**；确无行业术语的纯概念知识点，标注"本知识点无行业术语"并省略本节。
...
- 英文名与官方文档一致（不自行翻译造词）；一个概念多个叫法时给最常见的一个 + 括号列别名。
