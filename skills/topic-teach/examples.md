...
> 场景：`k8s 基础` Phase 5 产物三。展示一个"开放设计题"的完整结构——**重点是多解法 + 代价 + 递进路径，以及"先想后看"的折叠设计**。
...
> **怎么用**：每个场景先自己想"怎么做"，再展开看解法。先想后看，能力才长出来。
...
| 解法 | 能解决到 | 代价 | 适用边界 |
|------|---------|------|---------|
| CDN 缓存静态资源 | 拦截 ~90% 请求 | 动态内容无效 | 静态资源为主 |
| 应用层水平扩容 | 近线性扩展 | 需无状态 | 有状态要改造 |
| 数据层读写分离 | 读扩展 | 主从延迟 | 读多写少 |
| 缓存热点数据 | 挡读流量 | 一致性风险 | 容忍短暂不一致 |
| 限流 / 排队 | 保护后端 | 牺牲体验 | 突发洪峰 |
| 分库分表 | 大扩展 | 复杂度剧增 | 单表瓶颈 |
...
> **为什么这算"设计态"而不是"教程"**：它不教你"k8s 是什么"，而是让你在"流量翻 10 倍"这个真实压力下**自己做取舍**——6 个解法各有代价，推荐路径是"先便宜后贵"，知识点挂钩把解法接回课程，最后还反指经验层的坑。学习者先自己列解法，再对照：列得少说明整合不够，列了但说不出代价说明没真懂。

---

## 示例 15：场景解法库（非技术域理财）

> 场景：非技术域同样有场景解法库。结构同构——「解法一览」改为「路径一览」（按目标给多路径 + 后果核算）。展示"通胀来了怎么办"。
...
> **双轨差异一句话**：技术域「解法一览」问"能扛多少流量"，非技术域「路径一览」问"这笔钱什么时候用、能承受多少波动"——**核心结构（多选项 + 代价 + 递进 + 挂钩 + 边界）不变，变的只是选项的维度**。场景解法库不因"没有代码"而跳过。
