...
> 供 AI 参考"产物长什么样"，锚定格式。示例基于虚构的 `docker 网络模式`（速览）、`k8s 基础`（课程制阶段 1 课 2 的一个批次）、学习档案、结课实战项目与实战经验 / 排障速查手册 / 场景解法库，内容为示意，非完整教学正文。
...
- [示例 10：课程目录索引（书本式目录）](#示例-10课程目录索引书本式目录)
- [示例 11：综合实战项目（技术域 k8s）](#示例-11综合实战项目技术域-k8s)
- [示例 12：实战经验与排障速查手册（同一故障的两面）](#示例-12实战经验与排障速查手册同一故障的两面)
- [示例 13：非技术域排障手册（理财）](#示例-13非技术域排障手册理财)
- [示例 14：场景解法库（技术域 k8s）](#示例-14场景解法库技术域-k8s)
- [示例 15：场景解法库（非技术域理财）](#示例-15场景解法库非技术域理财)
- [示例 16：源码解析（Python 库）](#示例-16源码解析python-库)
...
> 📖 结论已按官方文档核对（核对于 2026-08-03 ｜ 来源：docs.docker.com/network）

## 速览大纲

本次讲清：Docker 的 5 种网络模式是什么、各自适合什么场景。路线：先看有哪些模式 → 再逐个对比 → 最后按场景选型。
...
> 📌 **一句话本质**：把"每个容器手动接网络"变成"建容器时选一个模式"。
> ⚖️ **处境对照**：模式没选对——容器之间 ping 不通、端口映射改了又改，排查半天；选对——`docker run` 加一个 `--network` 参数就位。

## 核心原理

### 一眼全局图（进入细节前先看一眼）

![五种网络模式解决什么问题](./assets/docker-net-modes-intuition.svg)

> 看图：左边是"每个容器各自接线、接法各异"，右边是"建容器时选一个模式、行为可预期"，差别在于网络从"逐个手工处理"变成"创建时一次决定"。
...
> 🗣️ **行话对照**：这 5 个模式名就是**行业标准叫法**（bridge / host / none / container / overlay）——**在哪遇到**：`docker run --network <模式>`、Compose 的 `network_mode:` 字段、`docker network ls`；排障时在 `docker inspect` 的 `NetworkSettings` 里能看到它们。
...
```
我刚看了 docker-networking/overview.md 关于 docker 网络模式的速览，
请给我一个 docker 网络配置的综合实战项目练手，或讲讲 docker 网络的实战经验、排障速查手册与场景解法库。
```
```
...
> 📖 结论已按官方文档核对（核对于 2026-08-03 ｜ 来源：kubernetes.io/docs/concepts/workloads/pods）
...
> 📌 **一句话本质**：把"一个个手动管理的容器"变成"一组一起调度、共享资源的容器"。
> ⚖️ **处境对照**：三个容器各自配网络、配存储、手工保证同机同生命周期；Pod 声明一次，调度、网络、存储由 k8s 一起保证。
...
![Pod 解决什么问题](../assets/pod-intuition.svg)

> 看图：左边是"每个容器单独安排、互相不认识"，右边是"一组容器住进同一个单元、共享网络与存储"，差别在于"一起调度"还是"各管各的"。

### 本课地图（分几步走）

| 步 | 这一步要解决什么（人话） | 对应知识点 |
|:--:|------------------------|-----------|
| 1 | 先弄清"Pod"到底是什么 | 知识点 1：Pod 概念 |
| 2 | 学会把想要的东西写下来交给 k8s | 知识点 2：Pod YAML |
| 3 | 解决"一个容器不够用、多个要协作" | 知识点 3：多容器共享 |

### 知识点 1：Pod 概念

> 🧭 第 1/3 步｜承接：容器能跑但各管各的（第二幕的冲突） → 本步：说清"一组容器一起调度"的单元是什么
...
- **Pod**：k8s 的最小调度单位——就是本课说的"合租房间"；在哪遇到：`kubectl get pods`、YAML 的 `kind: Pod`，所有工作负载控制器（Deployment / StatefulSet）最终都落到 Pod
- **Network Namespace（网络命名空间）**：容器间网络隔离的机制，同 Pod 容器共享它——就是本课说的"共享一根网线"；在哪遇到：进容器 `kubectl exec` 后 `localhost` 能直接访问同 Pod 的另一个容器
...
> 🧭 第 2/3 步｜承接：知道 Pod 是"合租房间"了，可房间长什么样、怎么向 k8s 描述它？ → 本步：学会把期望状态写成声明
> 本知识点关键点：四要素、容器字段、探针
...
> 🧭 第 3/3 步｜承接：一个容器的声明会写了，但"合租"的意义还没兑现 → 本步：多个容器怎么在同一 Pod 里协作
...
> 整课完成后，在接力提示词之后追加「课程导航」区块（**配实战的课首行加「练一练」；配解析的课在练一练之后加「🔬 源码解析」行**；上一课 / 下一课 / 返回目录；第一课省略上一课，最后一课下一课改为完成提示）：
...
> 课内 4.2 只留入口块（见示例 2）；实战正文独立成篇于 `应用实战/`（**仅通过「应用实战判定」的课才有**——纯概念 / 原理课不强配）——下面是同一份实战的完整落盘形态与一键跳转索引（片段）。
...
> **按需配套**：仅收录有实战场景的课（未列入的课为纯概念 / 原理类，无需实战）。
...
> 场景：`k8s-basics/` 主题下已落盘多课（课 1、课 2 已编写，课 3 未编写）+ 手册进行中。文件为 `k8s-basics/02-课程目录.md`，随教学进度更新。
> 书本式目录：已编写 = 可点击链接，未编写 = 纯文本；状态细节以 `00-学习档案.md` 为准。
...
> **维护要点**：Phase 1 落盘时初始化骨架（全部计划课 + 实战项目 / 实战经验 / 排障手册条目 = 纯文本）；Phase 2 每批完成后把对应课改为可点击链接；速览完成后追加链接条目；Phase 3 实战项目完成后改链接；Phase 4 手册完成后追加链接条目；Phase 5 完成后实战经验 / 排障手册 / 场景解法库三条目改链接。目录保持书本式，状态细节以 `00-学习档案.md` 为准。

---

## 示例 11：综合实战项目（技术域 k8s）

> 场景：课程制 `k8s 基础` 全部阶段讲完，进入 Phase 3。展示 `projects/三层Web应用灰度发布/README.md` 与 `设计决策.md` 的关键片段——**重点是知识点地图的跨阶段回指，和决策点的"真争议"写法**。

### README.md 片段

```markdown
# 实战项目：三层 Web 应用灰度发布平台

> 所属课程：k8s 基础 ｜ 学习目标：动手实操 ｜ 预计耗时：4-6 小时

## 🎯 一句话需求

把一个三层 Web 应用（前端 + API + 数据库）部署到 k8s，支持按流量比例灰度发布新版本，且出问题 30 秒内可回滚。

## ✅ 目标与非功能约束

- **功能目标**：① 三层应用在 k8s 跑通 ② 支持 5% → 50% → 100% 的流量灰度 ③ 一键回滚
- **非功能约束**：
  - **错误处理**：任一 Pod 崩溃不影响整体可用（自愈 + 多副本）
  - **可维护性**：全部配置声明式（YAML 纳入版本管理），**禁止** kubectl 手工改线上
  - **性能**：滚动更新期间请求成功率 ≥ 99.9%
...
```markdown
## 决策 1：灰度方案选 Deployment 双副本集，还是 Ingress 权重分流

| 方案 | 做法 | 优点 | 缺点 |
|------|------|------|------|
| A · 双 Deployment + Service 权重 | 新旧各一个 Deployment，调副本数比例控流量 | 无需额外组件，任何 k8s 都能跑 | 粒度粗（副本数比例），流量不精确 |
| B · Ingress 权重分流 | Ingress 配 canary-weight 注解 | 流量比例精确（1% 可调） | 依赖特定 Ingress 控制器，版本差异大 |

**最终选择**：A

**选择理由**：本项目约束是"通用可跑 + 30 秒回滚"。A 不依赖任何第三方控制器，回滚就是调副本数，最快；B 的精确分流在 3 副本规模下用不上（1:2 已够用）。

**为此付出的代价**：流量比例只能是副本数的整数比（3 副本最小粒度 33%），做不到 1% 级灰度。

**什么时候该改选另一个**：若要求"先放 1% 流量观察 5 分钟"→ 改 B，此时精确分流的价值超过部署复杂度。
```

> **为什么这个决策点达标**：A 和 B 都有道理，换了人可能选 B。这就是"真争议"——若某个选择在所有约束下都明显更优，那它不配叫决策点。

---

## 示例 12：实战经验与排障速查手册（同一故障的两面）

> 场景：`k8s 基础` Phase 5（一次生成三份：08 实战经验 / 09 排障手册 / 10 场景解法库）。**本例聚焦 08 / 09 两份对"同一故障"的分工差异**——学习态讲机制，使用态只给动作；10 场景解法库见示例 14。

### 08-实战经验.md 片段（学习态 · 讲原理）

````markdown
### 故障模式 3：滚动更新期间请求大量 502

- **症状（可观测）**：`kubectl rollout status` 显示成功，但网关 5xx 错误率从 0.1% 飙到 15%，约 30 秒后自行恢复；Pod 事件无异常
- **根因**：Pod 被删除时，kubelet 发 SIGTERM 与 kube-proxy 更新转发规则是**并行**的。新 Pod 还没进 Endpoints，旧 Pod 已被摘掉，中间空窗期的请求打到正在关闭的旧 Pod → 502
- **排查路径**：
  1. `kubectl get endpoints {svc} -w` 观察更新期间 Endpoints 变化 → 若短暂为空或只含未就绪地址，确认本问题
  2. `kubectl describe pod {旧pod}` 看 `terminationGracePeriodSeconds` 与应用的实际优雅退出耗时 → 关闭快于规则同步则空窗期更长
- **修复**：给容器加 `preStop` 钩子（先 sleep 几秒再退出），让旧 Pod 等规则同步完再关；同时配 readinessProbe 确保新 Pod 真正可用才接流量
- **预防**：任何面向流量的 Deployment 都必须配 readinessProbe + preStop，这是滚动更新不丢请求的最低配置

> 对应速查条目：[09-排障速查手册.md · 滚动更新期间 502 飙升](09-排障速查手册.md)
````

### 09-排障速查手册.md 片段（使用态 · 只给动作）
...
|--------------------------|:------:|------|
| 滚动更新期间 502 飙升，30 秒后自愈 | 🔴 | **症状 3** |
| Pod 全部 Pending，事件 FailedScheduling | 🔴 | **症状 1** |
| 镜像拉取失败 ErrImagePull / ImagePullBackOff | 🟡 | **症状 2** |

---

## 🔴 症状 3：滚动更新期间 502 飙升，30 秒后自愈

**一眼识别**：`rollout status` 成功但网关 5xx 突增；更新结束后自动恢复；Pod 事件无异常

| 步骤 | 动作 | 预期 |
|------|------|------|
| **止血** | `kubectl rollout undo deploy/{name}` | 流量回旧版本，5xx 立刻回落 |
| 定位 1 | `kubectl get endpoints {svc} -w` | 更新期间 Endpoints 短暂不含就绪地址 = 本问题 |
| 定位 2 | `kubectl get deploy {name} -o yaml \| grep -A3 readinessProbe` | 无输出 = 未配就绪探针，确认根因 |
| 修复 | 补 readinessProbe + preStop sleep | 再次更新时 5xx 不再飙升 |
| 若无效 | 5xx 不随回滚回落 → 转 `debug` 现场排查（可能是网关或依赖方问题） | — |

> **预防**：面向流量的 Deployment 必配 readinessProbe + preStop
> **原理**：→ [08-实战经验.md](08-实战经验.md) · 故障模式 3
````

> **两份的分工一眼看得出**：经验层解释了"为什么会 502"（机制层，值得坐着读完）；手册只给"先回滚，再查 Endpoints 和探针"（动作层，30 秒内能照做完）。**不要把原理抄进手册**——紧急时读原理是浪费认知带宽；也不要把手册动作抄进经验层——学的时候只记动作学不到东西。

---

## 示例 13：非技术域排障手册（理财）

> 场景：非技术域主题同样有 QRH。结构与技域**同构**——"止血"是止损动作，"定位"是核实动作，只是把命令换成处置话术与核实渠道。

````markdown
# 个人理财 排障速查手册

> **怎么用**：遇到可疑情况别从头读，按索引找**现象**，翻到条目照做。
> **查不到**：→ 转 `debug` skill 现场取证（如具体账户异常的技术排查）。

## 🔎 症状倒查索引

| 一眼看到的现象 | 紧急度 | 翻到 |
|--------------|:------:|------|
| 接到自称银行/平台客服的电话，索要验证码或要求转账 | 🔴 | **症状 1** |
| 账户出现不明扣款 / 持仓数对不上 | 🔴 | **症状 2** |
| 持有的基金连续 3 个月跑输业绩基准 | 🟡 | **症状 3** |

**紧急度**：🔴 立即处理（资金可能受损）｜ 🟡 尽快处理（有风险未爆发）｜ ⚪ 有时间再看（优化项）

---

## 🔴 症状 1：自称客服索要验证码

**一眼识别**：来电自称银行 / 支付平台客服，要求提供短信验证码，或引导"转到安全账户"

| 步骤 | 动作 | 预期 |
|------|------|------|
| **止血** | 立即挂断；不提供任何验证码、密码、身份证号 | 骗局链路中断，资金未动 |
| 定位 1 | 用**官方 App / 官网公布的号码**主动回拨核实 | 官方查无此事 = 诈骗 |
| 定位 2 | 检查账户是否有异常登录 / 绑卡变更 | 有 → 转「症状 2 账户出现不明扣款」 |
| 修复 | 已泄露信息 → 改密码 + 解绑异常设备 + 冻结账户 | 账户重回可控 |
| 若无效 | 已发生资金转出 → 保留通话与转账证据，联系银行止付并报警 | — |

> **预防**：验证码 = 支付密码。任何机构都不会主动索要验证码。
> **原理**：→ [08-实战经验.md · 常见诈骗手法与识别信号](08-实战经验.md)
````

> **双轨适配要点**：非技术域的"可观测症状"换成**可识别的外部信号**（来电话术、账单异常、业绩对比），"动作"换成**处置话术与核实渠道**（挂断、官方回拨、查账户）。结构与技域完全一致，**不因"没有代码"就跳过这一 Phase**。

---

## 示例 14：场景解法库（技术域 k8s）

> 场景：`k8s 基础` Phase 5 产物三（**独立成册**）。展示一个"开放设计题"的完整结构——**多解法（效果按题定制对比）→ 各解法详解（含机制图 + 关键代码）→ 替代路线 → 递进路径**，以及"先想后看"的折叠设计。本片段来自 `场景解法库/场景-01-流量翻10倍.md`（完整落盘中另有 `INDEX.md` 导航）。

### 场景解法库/场景-01-流量翻10倍.md 片段

````markdown
# 场景 1：流量突然翻 10 倍（规模压力题）

**场景描述**：电商大促，QPS 从 500 涨到 5000，数据库连接池先被打满，接口开始超时，客服工单暴涨。

![三层防线方案对比](./assets/scene-01-traffic-10x.svg)

读图：从左到右是 CDN → 应用层 → 数据层三道防线，每一层只挡掉一部分流量——单靠任何一层都挡不住 10 倍。

**🔒 先自己想 30 秒**：第一反应是加机器？加缓存？

<details><summary>💡 提示（分层 / 分维度想）</summary>

入口层 / 应用层 / 数据层，分别怎么扛？哪些是"改配置"、哪些是"改架构"？

</details>

<details><summary>📖 展开解法</summary>

### 解法一览（效果按"挡住量级 / 一致性代价"对比）

| 解法 | 效果（挡住量级 / 一致性） | 代价 | 适用边界 |
|------|------------------------|------|---------|
| CDN 缓存静态资源 | ~90% 请求 / 无影响 | 动态内容无效 | 静态资源为主 |
| 应用层水平扩容 | 近线性 / 无影响 | 需无状态 | 有状态要改造 |
| 数据层读写分离 | 读扩展 ~2–3× / 主从延迟 | 改造数据访问层 | 读多写少 |
| 缓存热点数据 | 挡读 5–10× / 短暂不一致 | 一致性风险 | 容忍短暂不一致 |
| 限流 / 排队 | 保护后端（不扩容）/ — | 牺牲体验 | 突发洪峰 |

### 各解法详解（含关键代码）

#### 解法：缓存热点数据

把商品详情页的读请求挡在缓存层——库里只承接未命中的请求。

![缓存旁路读路径：命中直接返回，未命中才落到数据库](./assets/scene-01-cache-hotspot.svg)

读图：请求先到 Redis，命中即返回；未命中才走数据库并回填（`setex`）。**红框标出失效点**——热点 key 过期的那一瞬间，大量请求同时穿透到库（缓存击穿），`TTL` 这道"兜底防线"反而在那一刻变成风险源。

```python
# 读路径：先缓存后库（Cache Aside）
def get_product(pid):
    data = redis.get(f"product:{pid}")
    if data is None:
        data = db.query("SELECT * FROM products WHERE id=%s", pid)
        redis.setex(f"product:{pid}", 300, json.dumps(data))  # TTL 兜底一致性
    return data
```

> 为什么这么写：`setex` 的 TTL 是最后一道一致性防线——即使更新逻辑漏删缓存，5 分钟后也会自然收敛。

#### 解法：应用层水平扩容

![无状态扩容：副本由 Deployment 拉起，就绪后才进 Service 后端](./assets/scene-01-horizontal-scale.svg)

读图：Deployment 拉起多个副本，Service 只把流量发给 `readinessProbe` 通过的 Pod。**红框标出两处失效点**——① 探针缺失时未就绪的 Pod 直接被灌流量，副本越多雪崩越快；② 扩出来的 Pod 共享同一个数据库连接池，**瓶颈只是从应用层搬到了数据层**。

```yaml
# 关键：无状态 + 就绪探针（没就绪就接流量，扩得越快死得越快）
apiVersion: apps/v1
kind: Deployment
spec:
  replicas: 3
  template:
    spec:
      containers:
        - name: api
          readinessProbe:      # ← readinessProbe 缺失是扩容翻车高发点
            httpGet: { path: /healthz, port: 8080 }
```

> 这个解法的代价 / 坑：副本翻倍 = 资源成本翻倍，且新 Pod 有冷启动（拉镜像 + 就绪等待）——大促要**提前**扩，不是出事再扩。

### 替代路线（不用 k8s 侧的解法）

| 替代方案 | 思路 | 与本课程解法的差异 |
|---------|------|------------------|
| 云托管弹性 / Serverless | 流量来了按云平台策略拉起，运维交给云 | 不用自己管 Deployment / 伸缩策略，但冷启动与成本曲线不同 |
| 虚机 + 负载均衡 | 不开 k8s，传统三层部署 | 无调度层能力（自愈 / 滚动更新要自己搭），可控性不同 |

### 推荐路径（递进，不是一次全上）

1. 先 CDN + 热点缓存（成本最低，挡掉大头流量）
2. 应用无状态化 → 水平扩容
3. 数据层读写分离
4. 仍不够 → 分库分表 / 异步削峰
...
- 水平扩容 / 探针 → 阶段 2 课 3 Deployment
- 缓存一致性 → 阶段 3 课 6
- Service 流量分发 → 阶段 3 课 5

### 什么情况下此方案不适用

- 强一致性场景（如交易扣款）不能粗暴加缓存，先保证正确再谈性能
- 写多读少时读写分离无益，反而引入主从延迟

### 做错会踩的坑

→ 关联 [08-实战经验.md](../08-实战经验.md) 故障模式 3（滚动更新期间 502）：扩容时若没配 readinessProbe，新实例未就绪就接流量，流量越大死得越快

</details>
````

> **为什么这算"设计态"而不是"教程"**：它不教你"k8s 是什么"，而是让你在"流量翻 10 倍"这个真实压力下**自己做取舍**——解法各有代价（效果列按题定制对比维度），推荐路径是"先便宜后贵"，代码落到"关键处为什么这么写"，知识点挂钩把解法接回课程，替代路线回答"不这么干还有什么做法"。学习者先自己列解法，再对照：列得少说明整合不够，列了但说不出代价说明没真懂。
> **图与 INDEX**：**每个解法必配 1 张**——上例 2 个解法各 1 张（画清机制 + **红框标出失效点**），**只有文字 + 代码 = 该解法没讲透**；合格判据是*搬到本场景另一个解法下面就不成立*（防凑图）。**场景级总览图（本篇开头那张）属可选**，只在多条解法值得横向对比时才加，不与逐解法图重复。文件入 `场景解法库/assets/`，命名**英文 kebab-case**（如 `scene-01-cache-hotspot.svg`——**SVG 文件名禁用中文**，见「文件命名规范」）；**引用必须用图片语法**（写成 `[xxx.svg](…)` 链接式则图不渲染）。场景文件由 `场景解法库/INDEX.md` 统一导航（场景名 / 类型 / 覆盖知识点 / 链接）。

---

## 示例 15：场景解法库（非技术域理财）

> 场景：非技术域同样有场景解法库。结构同构——「解法一览」改为「路径一览」（按目标给多路径 + 后果核算），无代码（非技术域正当），「替代路线」改为「对照路线」（其他地方的常见做法 + 差异）。展示"通胀来了怎么办"。

### 场景解法库/场景-01-通胀高企.md 片段（理财）

````markdown
## 场景 1：通胀高企，存款购买力缩水（决策题）

**场景描述**：年通胀率 5%，10 万存款一年后实际购买力只剩约 9.5 万，跑不赢就持续缩水。

**🔒 先自己想 30 秒**：放余额宝？买理财？还是不动？

<details><summary>💡 提示（按目标 / 期限想）</summary>

先想清楚：这笔钱什么时候要用？能承受多大波动？——不同答案对应不同路径。
...
| 路径 | 长期回报 | 代价 / 风险 | 适用边界 |
|------|---------|-----------|---------|
| 存款 / 货基 | ~1-2% | 跑不赢通胀，实亏 | 3 个月内要用的钱 |
| 债基 / 固收+ | ~3-5% | 短期波动 | 1-3 年不用的钱 |
| 指数基金定投 | 长期 ~6-8% | 波动大，可能浮亏 | 3 年以上 + 能承受波动 |
| 权益类 | 长期更高 | 回撤可达 30%+ | 长期 + 高风险承受力 |

### 各路径详解（**表格是索引、详解是正文**——每条路径逐一小节展开）

#### 路径：指数基金定投

![定投的现金流与成本摊薄](./assets/scene-01-inflation-dca.svg)

读图：横轴是时间，柱子是每期固定金额买入，折线是净值——**净值低时同样的钱买到更多份额**，这就是摊薄成本的来源。**红框标出失效点**——若在净值低谷（灰色阴影）因恐慌停买或赎回，摊薄机制当场失效，浮亏变实亏。

每月固定金额买入宽基指数，用时间摊薄成本——不预测涨跌，只赚长期经济增长的钱。

- **为什么适合通胀场景**：权益类长期回报通常跑赢通胀，定投又抹平了择时风险。
- **代价 / 风险**：短期可能浮亏，且需要纪律——**跌时更要买**，这正是最反人性的地方。
- **边界**：只放 3 年以上不用的钱；1–2 年内要用的话，波动会逼你在低点卖出。
- **数字算给你看**：年化 7% 约 10 年翻倍（72 法则）；但若在 −30% 时割肉，浮亏就变成永久损失。

#### 路径：债基 / 固收+

![三类资产的收益与波动区间：债基夹在存款与权益之间](./assets/scene-01-inflation-risk-return.svg)

读图：横轴波动（风险）、纵轴长期回报——存款在左下（低波动低回报），权益在右上，债基夹在中间。**红框标出失效点**——"固收+"的"+"就是权益仓，市场大跌时它会跟着回撤且幅度明显大于纯债，"保本"预期会落空。

以债券打底、少量权益增厚，波动介于存款和股票之间。

- **为什么适合通胀场景**：收益高于货基，能部分对冲通胀。
- **代价 / 风险**：短期仍有回撤；"固收+"里的"+"就是权益，跌起来不比纯债温和。
- **边界**：不适配 3 个月内要用的钱（赎回可能亏损）。

### 推荐路径（递进）

1. 先留 3-6 个月应急金（货基，随时可取）
2. 1-3 年要用的钱 → 债基 / 固收+
3. 3 年以上不用的钱 → 指数基金定投（分散 + 摊薄成本）

### 对照路线（其他地方 / 工具的常见做法）

| 常见做法 | 思路 | 与本课程路径的差异 |
|---------|------|------------------|
| 全部放存款 | 只求保本 | 名义不亏、实际被通胀侵蚀（本例的反面） |
| 跟热点追高 | 哪个涨买哪个 | 忽略期限匹配与波动承受力，易追涨杀跌 |
...
- 短期要用的钱不能进权益类（可能被迫割肉）
- 不能承受波动就别追求高收益（收益和波动是同一枚硬币）

### 做错会踩的坑

→ 关联 [08-实战经验.md](../08-实战经验.md) 故障模式（追涨杀跌）：高位一把梭 + 亏损割肉，正是定投要避免的
</details>
````

> **双轨差异一句话**：技术域「解法一览」问"能扛多少流量"，非技术域「路径一览」问"这笔钱什么时候用、能承受多少波动"——**核心结构（多选项 + 逐条详解〔含机制图〕 + 代价 + 递进 + 挂钩 + 边界 + 替代/对照路线）不变，变的只是选项的维度**。代码"能加则加"（取广义：程序 / 命令 / 配置；纯流程 / 组织类无代码正当）；非技术域无代码，以「路径一览 + **各路径详解（含机制图 + 后果核算 / 数字算给你看）** + 对照路线」承载对比——**"只有一览表没有详解"同样不合格**（C1），**逐解法（逐路径）配图同理不免**（真无图可画的纯条款类才走豁免，且须写理由）。

---

## 示例 16：源码解析（Python 库）

> 场景：库类课程的源码解析篇（**独立成册**，随课生成；限库 / SDK / 框架类主题）。展示一篇完整解析的结构与讲法——**为什么读它 → 功能全貌（图）→ 源码直击 → 设计权衡 → 回扣用法**。示例课程：「Python 进阶」第 3 课《装饰器与缓存》。本片段来自 `源码解析/03-装饰器与缓存.md`。

### 源码解析/03-装饰器与缓存.md 片段
...
class Repo:
    @cached_property
    def items(self):
        return self._query()   # 只查一次
```

但一个反直觉的问题：它**没有**任何"缓存字典"——第二次访问为什么就不再调用 `_query()` 了？答案藏在"**非数据描述符**"这个冷门机制里——看懂它，你就同时看懂了一类 Python 惯用法的底层原理。

## 功能全貌

```mermaid
flowchart LR
    A[实例访问 repo.items] --> B{实例 __dict__ 里有 items 吗}
    B -->|没有| C[走 cached_property.__get__ 计算]
    C --> D[结果写进 instance.__dict__]
    B -->|有| E[直接返回实例字典里的值]
```

> 读图指引：关键在于 D——第一次计算的结果被"降级"存进了实例字典，从此挡住了后续的 `__get__`。
...
class cached_property:
    def __init__(self, func):
        self.func = func                              # 存原函数
        self.attrname = None                          # 属性名由 __set_name__ 回填
        self.__doc__ = func.__doc__

    def __set_name__(self, owner, name):
        self.attrname = name                          # ← 让描述符知道自己的名字

    def __get__(self, instance, owner=None):
        if instance is None:
            return self                               # 类属性访问 → 返回描述符自身
        val = self.func(instance)                     # 首次：调原函数计算
        instance.__dict__[self.attrname] = val        # ← 关键一步：写入实例字典
        return val
```

> 节选并简化（省略了 `__set_name__` 重名校验 / `__slots__` 报错等边界分支）——完整源码见 `Lib/functools.py`。

## 设计权衡

- **为什么不写"缓存字典"**：`instance.__dict__` 就是现成的缓存位——**写入实例字典后，属性查找优先命中字典，不会再走到 `__get__`**（非数据描述符不拦截实例字典）——零额外存储、零额外查找。
- **如果定义 `__set__`（变成数据描述符）**：每次访问都会走 `__get__`，就还得自己判断"算没算过"——多一层状态管理。**"不定义 `__set__`"本身就是设计决策。**
- **代价**：缓存不可失效（要更新得 `del instance.attrname`）、实例必须有 `__dict__`（`__slots__` 类不可用）、**非线程安全**（3.12 起移除了早先的内部锁——多线程下同一实例的 getter 可能重复运行）——这些限制正是实现决定的。

## 回扣用法

- **注意**：类属性访问（`Repo.items`）返回的是描述符对象而不是计算值——做类型标注时别标成返回值类型；
- **可以**：昂贵又只需算一次的属性（DB 查询 / 大计算）直接用 `@cached_property`——"写进实例字典后靠属性查找优先级挡住后续 `__get__`"这套机制文档没展开（只写了"缓存为普通属性"），但被官方测试覆盖为稳定行为；
- **别**：在 `__slots__` 类上用它（没有 `__dict__` 可写，直接报错）；要"可刷新缓存"的场景用普通 `property` 自己管理。

---

🔬 **看透标志**：能说清"第二次访问为什么绕过 `__get__`"，并说出"数据描述符 / 非数据描述符"在属性查找优先级里的位置。

> 回索引：[源码解析索引](./INDEX.md)
````

### `源码解析/INDEX.md`（一键跳转索引 · 片段）

````markdown
# Python 进阶 源码解析索引

> **一键跳转**：本课程全部源码解析——学到哪课就点哪篇；想深看"这个功能内部怎么实现"时从这里找。
> **按需配套**：仅收录有解析价值的课（核心功能值得深挖；未列入的课读文档即可）。
> 活文档：随课程进度更新（与 `02-课程目录.md` 同节奏）。
...
> **为什么这算"看透实现"而不是"源码阅读课"**：从学习者的**真实困惑**出发（"没有缓存字典，为什么就缓存了？"），一张图标出关键步骤，一段真实源码落在关键行，最后把理解**还回用法**（注意 / 可以 / 别）——不是逐行解说，而是"看懂这一个为什么"。
> **图与 INDEX**：本例含 1 张 Mermaid（调用链）；若讲"描述符协议在属性查找链里的位置"这类结构，按选型规则改用 SVG 入 `源码解析/assets/`；解析篇由 `源码解析/INDEX.md` 统一导航（课 / 解析的功能 / 覆盖知识点 / 链接）。
