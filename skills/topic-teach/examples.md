...
> 整课完成后，在接力提示词之后追加「课程导航」区块（上一课 / 下一课 / 返回目录；第一课省略上一课，最后一课下一课改为完成提示）：
...
> 场景：`k8s 基础` Phase 5 产物三（**独立成册**）。展示一个"开放设计题"的完整结构——**多解法（效果按题定制对比）→ 各解法详解（含关键代码）→ 替代路线 → 递进路径**，以及"先想后看"的折叠设计。本片段来自 `10-场景解法库/场景-01-流量翻10倍.md`（完整落盘中另有 `INDEX.md` 导航）。

### 10-场景解法库/场景-01-流量翻10倍.md 片段
...
> **图与 INDEX**：本例省略设计图（本场景改造跨多层，正式落盘建议配 `assets/scene-01-三层防线.svg`）；场景文件由 `10-场景解法库/INDEX.md` 统一导航（场景名 / 类型 / 覆盖知识点 / 链接）。
