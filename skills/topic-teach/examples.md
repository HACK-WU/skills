...
> 供 AI 参考"产物长什么样"，锚定格式。示例基于虚构的 `docker 网络模式`（速览）、`k8s 基础`（课程制阶段 1 课 2 的一个批次）、学习档案、结课实战项目与实战经验 / 排障速查手册，内容为示意，非完整教学正文。
...
```
我刚看了 docker-networking/overview.md 关于 docker 网络模式的速览，
请给我一个 docker 网络配置的综合实战项目练手，或讲讲 docker 网络的实战经验与排障速查手册。
```
```
...
> **维护要点**：Phase 1 落盘时初始化骨架（全部计划课 + 实战项目 / 实战经验 / 排障手册条目 = 纯文本）；Phase 2 每批完成后把对应课改为可点击链接；速览完成后追加链接条目；Phase 3 实战项目完成后改链接；Phase 4 手册完成后追加链接条目；Phase 5 完成后实战经验 / 排障手册两条目改链接。目录保持书本式，状态细节以 `00-学习档案.md` 为准。
...
> 场景：`k8s 基础` Phase 5，一次生成两份。**同一件事，两份产物的分工差异是重点**——学习态讲机制，使用态只给动作。
