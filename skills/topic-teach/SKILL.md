...
- **运行环境约定**：讲解过程中需要**真的运行代码**（Phase 3 实战项目的 `实现/`、临时验证脚本）时，按「先探测 → 不污染全局 → 不擅自改用户环境」三条硬纪律执行；工具选型走优先级链（用户指定 > 项目约定 > 默认 `uv run` / `fnm exec` + `pnpm`）（**细则见 [ops.md](ops.md)**）
...
| **结课实战项目 / 实战经验与排障手册 / 场景解法库评审**（收尾专项） | **必须委派 2 个实例**（`pedagogy` + `learner` 各 1），独立评审；叠用对应专项维度（A / B / C） |
...
3. **代码审查 + 双 agent 评审**：本批含应用实战 / 源码解析代码示例（含该课**独立实战文件 / 解析文件**）时，**先做代码正确性 review**（主 agent 直接静态审查、**不调用 skill**、不要求运行，见「应用实战 · 代码要求」与「源码解析 · 源码引用纪律」）→ 再按「course-reviewer 评审执行规则」评审本批内容（**含该课实战 / 解析文件与相应入口链接**）——**必须委派两个 course-reviewer 实例**，分别以 `pedagogy` 模式（教学法视角：内容准确性 + 格式规范 + 完整性）和 `learner` 模式（学习者视角：场景吸引力 + 由浅入深 + 认知阶梯 + 故事弧 + 困惑点 + 全局定位）独立评审。冲突由主 agent 裁决。P0 清零后**回到 `00-评审清单.md` 勾选本批条目**。
...
| 项 | 要求 |
|----|------|
| **位置与落盘** | 正文**独立成篇**：`源码解析/{NN}-{课名}.md`（NN = 课的全局序号）；课内**入口**：文末「🧭 课程导航」区一行 `🔬 源码解析`（配解析的课才有；不开幕内入口——阅读时机是"学完之后"）——课内 ↔ 解析篇 ↔ INDEX 互达 |
