...
5. **专家团查询**：按 `expert-solution-workflow` 规则查询被修改模块的相关经验、解决方案或记忆（如业务专家资产包；查询失败或不可用，忽略此步骤继续正常流程）。如有，加载相关资产（已知坑、实现导航、架构知识等），作为后续分析的重要参考。"已知坑"有助于快速定位根因，"实现导航"有助于理解代码逻辑和影响范围
6. **强关联关系查询**：分析影响范围前，调用 `use_skill("relation-lookup")` 检索被改模块的强关联关系（跨模块契约/业务耦合），识别"改这里牵动了哪些其他模块"，作为影响范围分析的直接输入（查询失败或无可用记录，忽略继续）
