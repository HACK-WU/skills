...
**🕸 文字树最小语法（跨 skill 统一，4 条）**：① `│`+`▼` 串联向下、`├─` `└─` 分支 / 并行；② 节点挂状态与关键信息，关键节点打标记（`← 根因` / `← 待验证` / `← 系统性缺陷`）；③ 多假设并列时分 `├─` `└─`、单链因果用 `│`+`▼`；④ 只画结构类信息（根因链 / 影响链 / 数据流），清单与判定仍用表格；节点 ≤15、层级 ≤4，不画决策路径与审批流。
...
**🕸 影响链文字树**（树看结构：谁牵动谁、几层；**受影响判定与说明仍看下方表**，树的级别标记须与表一致；调用方 ≥2 或有二级调用方时必画，最小语法见 Step 1.3）：

```text
`update_status()`（被修复函数）
├─ `handler()`（api.py:42）　← ❌ 确定受影响
│  └─ `dispatch()`（router.py:88）　← ⚠️ 需检查
└─ `scheduler()`（cron.py:100）　← ⚠️ 需检查
   └─ （无下游）
```
