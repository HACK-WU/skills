...
对 Bug 修复进行系统性分析——不只确认"修好了没"，更要回答"修了会影响什么"。借鉴 requirement-mining 的根因分析法，结合 code-review 的多维度影响检查，确保修复是**有底气的**而非"改了就完"。
...
**如果只有 Bug 描述没有修复代码**，先进入 Step 1 做根因诊断，帮助定位问题后，再对修复方案做影响分析。
...
当 AI 完成了代码修复（`write_to_file` 或 `replace_in_file`），且变更是 `fix:` 类型的 bug 修复时，主动提示用户：

```text
🔍 Bug 修复已完成。是否需要进行影响分析？
   → 分析修复的影响范围，检查是否会引入回归问题
   [是 / 否 / 稍后]
```

## 与 code-review 的协作

本 skill 与 code-review 互补，不替代：

| 维度 | bug-impact-analysis | code-review |
|------|---------------------|-------------|
| **关注点** | 修复"会影响什么" | 修复"写得对不对" |
| **时机** | 修复方案确定后 | 代码提交后 |
| **产出** | 影响面清单 + 回归风险评估 | 多维度审查意见 |
| **覆盖** | 调用链、数据流、场景 | 安全性、规范、性能、架构 |

**建议流程**：
1. AI 修复 Bug
2. → 运行 bug-impact-analysis 做影响评估
3. → 根据评估调整修复
4. → 提交后运行 code-review 做质量把关
5. → 如 code-review 发现复杂问题，触发 challenger 二次质疑

## 行为边界

- **不做代码修改**：只输出分析报告，不直接修改代码
