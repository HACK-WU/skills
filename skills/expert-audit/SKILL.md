...
| 条件必出 | 有数据落地 → `C4-数据流向与消费.md` 存在；**模块有测试时** → `implementation/06-测试.md` + `test/known-failures.md` 存在 | P1 |
...
- **测试信息**：`06-测试.md` 含测试可执行性（运行命令/环境依赖/已知失败）；`test/known-failures.md` 切面级标注（子专家/功能域各自标注）
