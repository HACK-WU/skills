---
name: ui-to-ascii
description: 把用户粘贴的 UI 设计稿 / 截图 / 原型图转成纯文本 ASCII 框线布局图并存入 Markdown（设计稿标注转为编号锚点 + 标注表）；也支持凭文字描述直接生成界面 ASCII 草图（直接输出、不落盘）。触发："把这个设计图转成文本"、"ui to ascii"、"存一下这个设计稿"、"帮我画个 XX 界面的 ASCII"。
---
