---
name: debug
description: 系统化排错：以运行时证据走"复现 → 假设 → 验证 → 定位 → 修复 → 回归"闭环，排查报错、测试失败、行为异常。触发："排查这个 bug"、"debug 一下"、"为什么会报错"、"定位一下问题"、"这个测试为什么挂了"，或被 api-testing / e2e-testing / demo-verify 转交定位根因。
---
...
**根因树（🕸 文字树，与上面结论配套输出；不替代证据链）**：

```text
{现象：报错 / 异常行为原文摘要}
  ▼
{直接原因：哪行代码的什么行为}（证据：{日志 / 测试 / 插桩输出}）
  ▼
{根本原因 ← 根因}（{设计遗漏 / 边界未考虑 / 依赖变化 / 环境差异}）
  ├─ {触发条件 1}（{值 / 状态}）
  └─ {触发条件 2}（{值 / 状态}）
  ▼
{引入点：commit / 变更（如适用）}
...
假设多次转向时，**排查路径**也用同款树：`{H1（证伪：证据）} → {H2（证伪：证据）} → {H3 ← 证实}`。
最小语法（跨 skill 统一，4 条）：① `│`+`▼` 串联、`├─` `└─` 分支；② 节点挂数字 / 状态，关键节点打标记（`← 根因` / `← 待验证`）；③ 并行标"（取 max）"、串行标"（串行相加）"；④ 只画结构类信息，清单仍用表格；节点 ≤15、层级 ≤4。
