---
name: e2e-testing
description: 对真实运行系统做端到端验证：按业务旅程编排步骤、跨步骤共享状态，验证跨组件最终一致状态。触发："跑一遍完整流程看看对不对"、"真实链路测试"、"端到端验证"、"全链路验证"、"检查跨系统终态"。
---
...
- **对存量前端做体验体检**（点一遍挑体验问题 / 看加载性能，非验收场景）→ 用 `frontend-walkthrough`
