---
name: task-dispatch
description: 将编码/开发任务拆分为多个独立子任务，并行分配给子agent执行，主agent负责合并集成。适用场景：(1) 实现多文件、多模块功能时需并行加速，(2) "并行开发"、"拆分子任务并行执行"、"task-dispatch"、"并行实现"、"多个子agent同时做"，(3) 修复多个独立bug，(4) 重构涉及多个独立模块的代码。输入任务描述，输出集成后的代码交付物。触发后由阶段0判断是否适合并行，不适合则走快速通道由主agent顺序执行。
---
