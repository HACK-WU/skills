...
强关联 = **改了 A 必须连带改 B（或强烈建议连带改）** 的跨模块耦合，用 ki 的 **group 分组**机制归档，保证按功能模块归档、可被整组检索。

## 核心 API（写入侧）

| API | 用途 | 关键参数 |
|-----|------|----------|
| `ki_bulk_sync_relation` | 批量写入强关联原子（推荐，比逐条 `ki_sync_relation` 快 N 倍） | `scope`（项目标识）、`items[]`（每条含 `group`/`relation`/`module_info`/`tags`） |
...
- **批量写入**：关联多时用 `ki_bulk_sync_relation` 一次写入多条（单次 ≤ 50 条，超出分批），比逐条 `ki_sync_relation` 快
- **一个功能模块涉及多个文件**：两端用列表列出所有相关类与文件位置，不限于单个；单向时 `{模块A}端` 即改动源、`{模块B}端` 即被影响方；双向时两端互为源目标，标签保持中性
- **校验**：写入后用 `ki_query_group`（group = `关联关系/{功能模块名}`，**`mode="full"`**）确认已归入正确子分组，抽查 1-2 条；**默认 `mode="hot"` 只返回最热 5 条**，新写入的关联若不在其中会被误判为写入失败
