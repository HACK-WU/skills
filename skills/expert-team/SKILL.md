---
name: expert-team
description: 派出多专家子 agent 并行深挖单个业务模块，产出含"契约层（黑盒使用文档）+ 实现层（白盒技术文档）"两层资产的"业务专家"包，供后续直接使用或导航代码。大型模块可按功能拆分为子专家。落盘到 .module-experts/。触发短语："掌握这个模块"、"深度分析 xxx 模块"、"梳理这个模块"、"建模块专家团"、"module expert team"，或需要对某业务模块建立深度、持久、可复用的领域专家资产时。
---
...
**功能**：模块范围扫描（AI 自行判断）→ 评估是否拆子专家 → 知识库优先查 → 派出多专家子 agent 并行产出实现层 Wiki 文档 → 主 agent 提炼契约层（含真实代码示例）→ 合成专家入口 → 校验 → auto-review 评估审核+优化 → 落盘到 `.module-experts/{中文业务专家名}/`。
...
Step 5 自检通过后，调用 [auto-review](../auto-review/SKILL.md) skill 对全部专家资产（含子专家）进行质量审查与优化闭环：
