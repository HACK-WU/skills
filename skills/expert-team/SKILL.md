...
18. **模块间强关联必记**：专题记忆写入后调用 `strong-relation` skill（Step 8.5）识别该模块与外部模块的强关联（契约/业务耦合），按功能模块分组写入 ki-search（统一父分组「关联关系」+ 动态子分组，tags 统一为 `relation`），解决 code review / 修改时"改了 A 不知道必须连带改 B"的风险
...
专家/专题落盘后，**必须**写一条"专题记忆"到 ki 记忆（用 `ki_store`，scope 取项目标识），让 AI 在任何对话中**未加载专家资产前**也能快速检索到"有这个模块的知识 + 关键入口 + 去哪找详细"。这是解决"专家资产闲置"的关键——资产很全但只在 AI 主动查 `expert-lookup` 时被用上，专题记忆把资产从"被动待查"变为"主动可达"。

**专题记忆定位：路标 + 快查卡，绝不缩水复制专家资产**。只存"资产存在感 + 关键入口 + 极少数高频原子"，不存知识本体。全量复制会造成双源漂移（专题记忆与专家资产不一致），是生态系统评审判定的有害冗余（`内置 memory + ki 记忆 + 专家资产` 等量重复）。

**专题记忆 8 字段模板**（每条极短，≤ 200 字）：
...
**格式硬约束（防污染 ki-search 检索）**：专题记忆写入 ki-search 时，**不得包含 yaml 元数据**（`---` frontmatter）、**不得使用 `##`/`###` 等 md 标题**、不得含 `|` 表格等 markdown 结构语法；统一用模板的**纯文本行 + `-` 列表**。字段内的补充说明（如关键代码位置的展开）用普通文本/列表，**禁止套用 `##` 章节结构**（如 `## 关键文件`）——yaml 和 md 标题会污染向量化，降低 ki-search 检索效率。

**字段填充来源**：逐条从已产出的专家资产提炼，禁止编造；任一字段为空则写「无」不硬凑。

**ki 不可用降级**：ki 不可用（未配置 `ki_store`/索引服务不可达）时，**跳过写记忆并在 Step 9 验收报告标注**，不阻塞专家落盘（对齐 Step 2 的既有降级约定，避免"必须"阻塞流程）。

**维护规则（防双源漂移）**：专题记忆是专家资产的**弱副本路标**，资产变更必须同步记忆：
- **新建专家/专题** → 写一条专题记忆（本步）
...
专题记忆写入后，**调用 `strong-relation` skill** 识别并记录该模块与**外部模块**的强关联（契约耦合 + 业务耦合），写入 ki-search 并按功能模块分组（统一父分组「关联关系」+ 按功能模块动态创建的子分组；tags 统一为 `relation`，供查询侧过滤）。

**定位**：专题记忆（Step 8）记录"模块内部知识路标"；`strong-relation` 记录"模块间强耦合"——解决 code review / 修改时改了 A 不知道必须连带改 B 的风险。两者互补，同存 ki。

**执行**：按 `strong-relation` 的流程执行（环境检测 → 定位模块 → GitNexus+源码识别关联 → 判定方向强度 → 分组写入 ki → 校验）。
...
- **ki 记忆格式硬约束**：写入 ki-search 的记忆内容（专题记忆、强关联等）禁用 yaml 元数据（`---` frontmatter）和 `##`/`###` 等 md 标题、`|` 表格；统一纯文本 + `-` 列表，保证 ki-search 向量检索效率
