...
**执行**：调用 `use_skill("ki-memory-write")` 的**专题记忆策略**，按该策略的 8 字段路标模板 + 维护规则写入 ki。ki 写入细节（8 字段模板、`ki_store` API、格式硬约束、维护规则）一律以 `ki-memory-write` 的 `reference-topic-memory.md` 为准，本步不重复声明。
...
- 写专题记忆（Step 8）与强关联（Step 8.5）的 ki 写入动作，统一走 `use_skill("ki-memory-write")`
