---
name: review-panel
description: 启动多个子 agent 组成评审团，对方案/代码/设计做多角色评审（双挑战者独立质疑 + 三评委评分讨论 + 主 agent 法官研判），产出含原始方案、反对意见、评分与最终判断的评审报告。仅在用户明确要求使用"评审团"、"review panel"时启用。
---
...
2. **写入评审报告**到需求目录后，调用 `req update` 注册文档关联：
