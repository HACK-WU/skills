---
name: expert-lookup
description: 查找并复用已沉淀的"业务专家"资产包（由 expert-team 生成，落盘于 .module-experts/）。支持多专家同时加载（最多3个），按使用意图分层加载（使用→契约层，深入→实现层+源码）。支持子专家查找，使用中发现不一致时可受限增量更新契约层。触发短语："有没有这个模块的专家"、"找一下告警相关的专家"、"这个模块有现成资料吗"、"查专家"、"expert lookup"，或在 AI 接手某业务模块、排查某领域问题时自动触发。
---
