...
4. 查 ki 专题记忆（Step 8 落盘的路标）→ 命中即知晓"有此专家 + 关键入口"
...
- **专题记忆同步**：expert-team 落盘专家后写 ki 专题记忆（8 字段路标）；本 skill 使用中可受限修正其路标字段（Step 6 权限内，字段级），职责/路径大改走 expert-team；记忆只存路标不存专家本体，避免双源漂移；**修正记忆时遵守格式硬约束**（禁用 yaml 元数据、`##`/`###` 等 md 标题，统一纯文本 + `-` 列表，防污染 ki-search 检索）
