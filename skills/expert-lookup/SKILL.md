---
name: expert-lookup
description: 在 AI 或用户需要了解/使用某个业务模块时，查找并复用已沉淀的"业务专家"资产包（由 expert-team 生成，落盘于 .module-experts/）。支持多专家同时加载协同工作（最多3个）。按使用意图分层加载：使用场景只看契约层（C0~C3，黑盒使用文档含真实代码示例，放在专家根目录）；深入模块时用 implementation/ 文档导航代码路径再读源代码确认——契约用于'使用'，implementation/ 用于'导航'，源代码用于'确认'。支持子专家查找。使用中发现契约与实际代码不一致或不够详细时，对契约层进行受限增量更新，形成"使用即维护"的闭环。触发短语包括："有没有这个模块的专家"、"找一下告警相关的专家"、"这个模块有现成资料吗"、"查专家"、"expert lookup"，或在 AI 接手某业务模块、排查某领域问题、需要使用模块能力时自动触发。
---
...
**目的**：在 AI 或用户需要了解/使用某个业务模块时，快速查找是否已有沉淀的"业务专家"资产包，**支持多专家同时加载协同工作（最多3个）**，按使用意图分层加载避免无谓读实现代码，并在使用中发现契约过期或缺漏时**自动增量更新**资产，形成"使用即维护"的闭环。

**核心能力**：
1. **查找专家**：读取 INDEX.md → 关键词匹配（含子专家）→ 读 agent.md 确认 → 加载专家资产
2. **多专家协同**（最多3个）：任务涉及多个域时，同时加载多个专家的契约层协同工作；独立子任务可并行派子 agent
3. **按意图分层加载**：使用/调用→契约层（根目录 C0~C3）；诊断/排查→C0+已知坑+implementation/03-数据流转（导航）+读源代码确认；修改/扩展→C0+implementation/01-架构+02-实现（导航）+读源代码修改
...
每个专家子目录含 `agent.md`（名片）+ 契约层文档（根目录：`C0-使用总览.md`~`C3-代码示例-{topic}.md`）+ `implementation/`（实现层：`01-架构.md`~`07-运维.md`）+ `CHANGELOG.md`（变更日志）。父专家可含 `sub-experts/`（子专家，仅一级）。
...
`INDEX.md` 中每条记录形如（由 `expert-team` Step 6 维护）：
...
### Step 3：关键词匹配（专家级粗筛，含子专家）

将 Step 1 的关键词与 `INDEX.md` 中每条记录的**专家名 / 模块根 / 契约层 / 实现层文档 / 子专家名**进行匹配：

**匹配规则**：
- 精确匹配优先：关键词与专家名 / 模块根 / 子专家名完全一致
- 包含匹配：当前关键词包含某记录的专家名 / 模块根 / 子专家名
...
对 Step 3 命中的候选专家（含子专家），按匹配度从高到低**逐个核对**：

1. **必须实读**该候选的 `agent.md` 全文：
   - **一句话职责**：负责的业务领域
   - **负责的模块**：模块根路径与职责
   - **何时找这个专家**：典型使用场景
...
2. **核对是否覆盖当前需求**：将当前问题与"一句话职责 / 负责的模块 / 何时找这个专家"比对
   - **agent.md 不存在、为空或缺关键字段** → 跳过并记录原因
   - **符合** → 标记为采用候选
   - **不符合** → 跳过，继续核对下一个
3. 核对完毕，按结果分流：
   - **1 个符合** → 进入 Step 5 单专家加载
...
| 级别 | 判定标准 | 处理方式 |
|------|----------|----------|
| **即时（P0）** | 照契约使用**一定出错**：参数类型/数量不符、返回结构不符、方法不存在、约束缺失导致必踩坑 | 求解中当场修正，向用户声明，继续求解 |
...
2. 与已加载契约比对
3. 发现差异 → 定级
4. **即时级**：暂停 → 修正契约 → 声明并确认 → 写 CHANGELOG → 继续
...
2. 查 expert-lookup（本 skill）
   → 命中多个 → 多专家协同加载（Step 5，最多3个）
   → 命中单个 → 单专家按意图加载（Step 5）
...
**匹配专家**：{专家1名}（+ {专家2名}，... 如多专家，最多3个）
**存储层级**：项目级 / 用户级
**匹配关键词**：{命中的关键词}
**识别意图**：使用/调用 | 诊断/排查 | 修改/扩展 | 待确认
**加载模式**：单专家 / 多专家协同 / 多专家并行
**加载资产**：{按意图加载的资产清单}
**路径**：{专家目录路径}
...
### 包含的资产
- 契约层：C0-使用总览, C1-能力契约, C2-使用流程, C3-代码示例-xxx（或「暂无」）
- 实现层：implementation/01-架构, implementation/02-实现, ...（或「暂无」）
...
| skill | 关系 |
|---|---|
| **expert-team** | **本 skill 查找其产出的专家资产；找不到时建议调用它创建。本 skill 可对契约层做受限增量更新，大改仍走 expert-team** |
...
- **受限增量更新**：仅允许补充/修正单条契约或流程条目（含代码示例），禁止删除/重写整篇、新增/删除切面文件、修改 agent.md 职责；大改走 expert-team
...
- **变身不篡改资产**：变身为目标专家仅加载其资产用于求解；增量更新仅限契约层受限范围
