...
1. 识别变更触碰的数据实体（集合/记录/字段），画出其修改前的「生产 → 转换 → 消费」链
2. 对比修改后的链路，逐跳回答守恒三问（流向/数量/内容）
3. 任一问命中变化 → **枚举该数据的所有消费方**（grep 调用点/订阅方/下游表读取方），逐个判定是否受影响
4. 判定规则：变化且消费方受影响 → ❌ Bug（P0/P1 按影响面定级）；变化但所有消费方均兼容 → ⚠️ 标注待确认（可能是有意为之，需与提交意图交叉验证）；无变化 → ✅
5. 静态无法确认实际数据量变化时（如过滤条件依赖运行时数据分布），标注 `[需运行验证]` 并建议接力 debug / demo-verify
...
**0.10.2 审查动作（逐条搭车变更立案）**：

1. **围栏检查**：原写法是不是故意的？依次查：原代码行附近注释（含 HACK/NOTE/不要优化类提示）→ `git log -L`/`git blame` 看原写法的提交信息（尤其是 bug 修复提交留下的写法）→ 是否有测试用例专门锁定原行为
2. **同等强度审查**：对搭车变更执行 0.6 全局回扣与 0.9 守恒三问，审查标准与主变更完全一致
3. **判定**：破坏全局判据/数据守恒 → ❌ P0（局部看是优化、全局看是 bug）；无法证明原写法非故意 → ⚠️ 建议拆分独立提交单独评审（举证责任在改动方）；已证非故意且全局无影响 → ✅ 放行，但仍建议拆分提交保持变更原子性
...
| 🚙 搭车变更未经证明 | 🟡 P1（破坏全局则 🔴 P0） | 围栏检查（注释/blame/测试）→ 无法证明原写法非故意则建议拆分独立提交 → 破坏全局判据则要求回退 |
