...
进入 Review 前，需先拿到可审查的 diff。根据用户的表述，按以下场景选择对应命令获取变更内容（统一加 `--no-pager` 避免分页器打断流程，`-U15` 可展示更宽的前后上下文）：

| 用户表述 / 场景 | 含义 | 获取命令 |
|----------------|------|----------|
| "查看最后 N 个提交的修改" | 最近 N 个提交的整体 diff | `git --no-pager diff -U15 HEAD~N..HEAD` |
| "查看某次提交 abc1234 的改动" | 单个 commit 的变更 | `git --no-pager show -U15 abc1234` 或 `git --no-pager diff -U15 abc1234^..abc1234` |
| "查看从 a1b2 到 c3d4 之间的修改" | 两个 commit 之间的变更 | `git --no-pager diff -U15 a1b2..c3d4` |
| "对比我的分支和 master 的改动" | 当前分支相对目标分支的累计变更 | `git --no-pager diff -U15 master...HEAD`（三点：仅本分支独有变更）<br>或 `git --no-pager diff -U15 master..HEAD`（两点：含目标分支新提交） |
...
| "看某个提交里某文件改了什么" | 单文件单提交 | `git --no-pager show -U15 abc1234 -- <文件路径>` |

**命令要点说明**：

- `HEAD~N..HEAD`：前开后闭区间，包含 HEAD 在内最近 N 个提交的所有变更（最常用）。
- 三点 `...` vs 两点 `..`：
  - `A...HEAD`（三点）= 以 A 与 HEAD 的分叉点为基准，仅显示本分支独有的变更，**最适合分支对比 / PR 审查**。
  - `A..HEAD`（两点）= 显示 A 之后、HEAD 之前的所有提交差异（包含 A 之后的对方提交），可能造成范围过大。
- `-U15`：上下文行数，默认 3 行往往不足以理解语义，建议 10~20 行；安全/语义检查需要更多上下文时可加大。
- `--no-pager`：直接输出到结果流，避免被 `less` 等分页器阻断，便于后续流程继续处理。
...
- **专家资产辅助**：审查中等及以上变更时，主动查询被修改模块的业务专家资产（`expert-lookup`），复用已知坑、能力契约、架构知识辅助审查，提高语义推断和一致性检查的准确性
- **分层并行**：标准模式及以上时，将 7 个审查维度分为「独立维度」和「依赖维度」两组——独立维度用 `task-dispatch` 并行派发子 agent，依赖维度（语义一致性 + 架构）由主 agent 串行执行。共享语义声明表作为子 agent 的输入约束，主 agent 负责汇总和跨维度一致性校验
...
### 🧭 阶段 1.1：专家团查询（按审查深度分层触发）

#### 触发条件

**仅当变更规模 ≥ 6 行时触发**（直接从 diff 统计净增+净删行数，不含纯空行和纯注释行）。微小变更（≤5 行，快速模式）跳过，避免过度开销。

> 变更规模的判定与阶段 3 的「审查深度自适应」表格保持一致。若变更涉及多个被修改模块，仅查询核心变更模块（最多 2 个）的专家。

#### 查询流程

1. 从被修改的核心文件中提取模块关键词（模块路径、功能域名称、核心类名等，3-5 个关键词）
2. 调用 `expert-lookup` 技能查找是否有相关的业务专家资产包（如调用失败或不可用，忽略此步骤继续正常流程）
3. 如命中专家，按审查深度分层加载：

| 审查模式 | 加载资产 | 用途 |
|----------|----------|------|
| 🔍 标准模式（6-50行） | C0（使用总览 + 已知坑）+ C1（能力契约） | 为阶段 1.2 意图推理提供「模块原本能做什么」的参考基线；为维度 0 语义一致性检查提供公开方法的参数/返回/异常契约基准 |
| 🔬 深度模式（51-200行） | C0 + C1 + implementation/01-架构 + implementation/02-实现 + implementation/03-数据流转 | 额外辅助维度 4 架构设计评审、维度 2 的深层 Bug 模式识别、维度 0.6 整体语义影响分析 |
...
| 专家资产 | 辅助的审查阶段 | 具体用途 |
|----------|--------------|----------|
| C0 能力清单 + 边界 | 阶段 1.2（意图推理） | 了解模块原本的能力范围和边界，辅助判断被改代码的功能意图是否合理 |
...
> **核心原则**：专家资产是**辅助参考**而非权威来源。代码的实际行为永远是最权威的。当专家契约与代码行为冲突时，以代码为准，在报告中标注「专家契约可能过期，建议用 `expert-lookup` 增量更新」。

#### 未找到专家时

1. 不阻塞审查，正常进入阶段 1.2
2. 若审查中发现模块难以理解，可建议用户用 `expert-team` 为该模块创建专家资产包
...
| 业务专家 C0/C1 资产 | 🟢 中（如有） | 专家契约层描述了模块的能力边界和公开方法的参数/返回/异常契约，可作为可靠的意图参考基准（**仅当阶段 1.1 命中专家时可用**） |
...
| 变更规模                | 审查模式     | 专家团查询                     | 执行策略                       | 执行维度                       | 报告详细度                       |
| ----------------------- | ------------ | ------------------------------ | ------------------------------ | ------------------------------ | -------------------------------- |
| 🔹 微小变更（≤5 行净增） | ⚡ 快速模式   | 不查询                         | **串行执行**（无需并行开销）   | 0 语义 + 1 安全 + 2 Bug        | 仅列出发现问题，不展开维度表     |
| 🔸 中等变更（6~50 行）   | 🔍 标准模式   | 查询，加载 C0 + C1             | **分层并行**（波次1并行+波次2串行） | 全部 7 维度                    | 完整报告模板                     |
| 🔶 大型变更（51~200 行） | 🔬 深度模式   | 查询，加载 C0+C1+架构+实现+数据流转 | **分层并行**（波次1并行+波次2串行） | 全部 7 维度 + 架构影响传播分析 | 完整报告 + 调用链影响图          |
| 🔴 巨型变更（200+ 行）   | ⚠️ 先建议拆分 | 先建议拆分，审查时按实际规模复用规则 | 拆分后按实际规模复用上述策略   | 先执行阶段 0 门禁              | 建议拆分为多个 commit 后逐次审查 |

> 变更规模以「净增 + 净删行数」计，不含纯空行和纯注释行。若变更包含新文件，该文件按全部行数统计。
> 
> **专家团查询说明**：`expert-lookup` 调用失败或未找到专家时忽略，不阻塞审查。专家资产是辅助参考而非权威来源——当专家契约与代码实际行为冲突时，以代码为准，标记「契约可能过期」。
> 
> **分层并行说明**：详见下方「并行派发策略」章节。微小变更不并行（开销 > 收益）；标准及以上模式启用分层并行。

#### 🔀 并行派发策略（标准模式及以上启用）

> **核心设计**：code-review 的 7 个维度中，5 个维度相对独立可并行，2 个维度依赖语义上下文必须串行。分层并行在保留「语义一致性横跨所有维度」核心价值的同时，通过 `task-dispatch` 加速独立维度的审查。

##### 维度分组

| 分组 | 维度 | 特征 | 执行方式 |
|------|------|------|----------|
| **波次1：独立维度** | 1️⃣ 安全 + 3️⃣ 规范 + 2️⃣ Bug + 5️⃣ 性能 + 6️⃣ 测试 | 检查项相对自洽，不依赖其他维度的结论 | `task-dispatch` 并行派发子 agent |
| **波次2：依赖维度** | 0️⃣ 语义一致性 + 4️⃣ 架构设计 | 需要综合其他维度的发现，做跨维度一致性校验 | 主 agent 串行执行 |

##### 执行流程

```
阶段 0/1/1.1/1.2/1.5：主 agent 串行完成
  ↓ 产出共享上下文（见下文「共享上下文打包」）
  ↓
┌─────────────────────────────────────────────────┐
│ 波次1：独立维度并行（task-dispatch）            │
│                                                 │
│  子agent-A  → 维度1 安全 + 维度3 规范           │
│  子agent-B  → 维度2 Bug + 维度5 性能            │
│  子agent-C  → 维度6 测试                        │
│                                                 │
│  每个子agent接收：共享上下文 + 对应维度的检查清单│
└─────────────────────────────────────────────────┘
  ↓ 各子agent返回维度审查结果
  ↓
┌─────────────────────────────────────────────────┐
...
波次1的每个子 agent 必须接收以下共享上下文，确保审查基准一致：

| 共享内容 | 来源 | 用途 |
|----------|------|------|
| **功能意图推测**（权威语义） | 阶段 1.2 用户确认 | 子 agent 据此判断代码行为是否符合意图 |
| **语义声明表** | 阶段 1.5 提取 | 子 agent 据此检查语义一致性（尤其是维度 2 Bug 的语义相关风险） |
| **diff 内容 + 完整文件** | 阶段 1 获取 | 子 agent 审查的实际对象 |
| **受影响代码清单** | 阶段 1.2 步骤 A | 子 agent 据此确定审查范围 |
| **专家资产**（如有） | 阶段 1.1 加载 | 子 agent 据已知坑和契约辅助审查 |
| **语言/运行时版本** | 阶段 0.4 识别 | 子 agent 切换到对应语言的检查清单 |

> 子 agent **不重新执行**阶段 0/1/1.2/1.5，直接消费主 agent 的产出。这避免了重复推理，也保证了所有维度基于同一套语义基准。

##### 子 agent 分组策略

按维度间的耦合度分组，减少子 agent 间的信息依赖：

| 子 agent | 负责维度 | 分组理由 |
|----------|----------|----------|
| **子 agent-A** | 1️⃣ 安全 + 3️⃣ 规范 | 两者都关注代码的「正确写法」，检查项有重叠（如命名规范 vs 安全命名） |
| **子 agent-B** | 2️⃣ Bug + 5️⃣ 性能 | 两者都关注代码的「运行时行为」，Bug 模式和性能隐患常共生（如循环内 I/O） |
| **子 agent-C** | 6️⃣ 测试 | 独立性最强，单独成组；需要对照语义声明表验证测试是否覆盖意图 |

> 分组为 3 个子 agent 而非 5 个，平衡了并行度和协调成本。若变更规模极大（深度模式），可将每个维度拆为独立子 agent（最多 5 个并行）。

##### 子 agent prompt 要点

每个子 agent 的 prompt 必须包含：
1. **角色定义**：你是 code-review 的 {维度名} 审查专家
2. **共享上下文**：上述打包的共享内容
3. **维度检查清单**：对应维度的完整检查项（从 SKILL.md 中提取）
4. **输出格式**：按统一的结构化格式返回发现的问题（含行号、代码片段、问题描述、严重级别、修复建议）
5. **约束**：只审查分配的维度，不越界；发现跨维度问题时标注「⚠️ 跨维度线索」而非自行处理

##### 汇总与跨维度一致性校验

主 agent 收到所有子 agent 的结果后：

1. **去重**：同一代码位置被多个维度报告的问题，合并为一条，标注涉及的维度
2. **冲突解决**：当不同维度给出矛盾建议时（如安全建议加密、性能建议避免加密开销），主 agent 基于语义声明表判断哪个更符合功能意图
3. **跨维度线索追踪**：子 agent 标注的「⚠️ 跨维度线索」由主 agent 在波次2中深入分析
4. **语义一致性综合**：维度 0 基于所有维度的发现，做最终的语义一致性判定

##### 降级策略

- `task-dispatch` 不可用或失败 → 降级为串行执行所有维度（原流程）
- 子 agent 返回超时或结果异常 → 主 agent 接管该维度的审查
- 共享上下文过大（超过上下文窗口 50%）→ 精简共享内容，仅保留功能意图 + diff + 受影响代码清单

#### 🔑 维度 0：语义一致性检查（最高优先级，波次2 · 主 agent 串行执行）
...
4. **专家 C1 契约 vs 实际实现**（当阶段 1.1 命中专家时，将 C1 中的参数/返回/异常/约束契约作为额外的语义基准，逐项与修改后的代码行为对比）
...
#### 2️⃣ Bug 与逻辑风险 🐛（波次1 · 子 agent-B 并行）
...
- **专家已知坑交叉对比**（当阶段 1.1 命中专家时）：逐一核对 C0 中的已知坑清单，检查本次修改是否重蹈历史上已知的问题模式。若命中已知坑，标注 P0 并引用专家 CHANGELOG

#### 3️⃣ 代码规范 & 语言惯用法 📐（波次1 · 子 agent-A 并行）
...
- **专家架构对齐**（当阶段 1.1 命中专家且深度模式时）：以 implementation/01-架构 描述的模块分层、依赖图、子模块边界为基准，检查修改是否违反架构约束（如将底层实现细节泄露到上层、绕过既定的分层调用路径等）

#### 5️⃣ 性能隐患 ⚡（波次1 · 子 agent-B 并行）
...
20. **专家资产按需加载**：仅标准模式及以上（变更 ≥ 6 行）时查询专家团，微小变更跳过以减少开销。查不到不阻塞审查。专家资产是辅助参考——C0 已知坑用于交叉对比 Bug 模式，C1 契约用于语义一致性基准，implementation/ 架构/实现/数据流转仅在深度模式加载。**代码实际行为永远是最权威的**，当专家契约与代码行为冲突时以代码为准
21. **分层并行触发条件**：仅标准模式及以上启用分层并行；微小变更（快速模式）串行执行，避免并行开销超过收益。`task-dispatch` 不可用时降级为全串行，不阻塞审查
22. **语义一致性不可并行**：维度 0（语义一致性）和维度 4（架构）必须由主 agent 在波次2串行执行——它们需要综合波次1所有维度的发现做跨维度一致性校验。禁止将这两个维度派发给子 agent
23. **子 agent 共享上下文**：波次1的子 agent 必须接收主 agent 在阶段 0/1/1.2/1.5 产出的共享上下文（功能意图、语义声明表、diff、受影响代码清单、专家资产），禁止子 agent 重新执行这些阶段。这保证所有维度基于同一套语义基准
24. **子 agent 不越界**：子 agent 只审查分配的维度，发现跨维度问题时标注「⚠️ 跨维度线索」交由主 agent 处理，不自行解决
25. **汇总去重与冲突解决**：主 agent 汇总时必须去重（同一位置多维度报告的问题合并）并解决冲突（不同维度矛盾建议时基于语义声明表判断）。跨维度线索在波次2深入分析
