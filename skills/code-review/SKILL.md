...
→ 阶段 2.2 强关联关系查询 → 阶段 2.5 需求上下文感知 → 阶段 3 功能意图推理
...
**执行**：调用 `use_skill("ki-memory-lookup")` 的**强关联策略**（提取模块关键词 → 精确列举该模块全部强关联 → **逐条**核对）。ki 查询参数细节一律以 ki-memory-lookup 为准，本阶段不重复。

> **强关联必须全量核对，不可只挑"看起来相关"的**：relation 名只含两端模块名、**不含强度与方向**，无法据此判断"必改"还是"可选"——挑着看恰恰会漏掉最该看的那条。且强关联**漏一条 = 漏改一个模块**，这是漏审风险，不是覆盖率取舍。
...
- **决策记忆核对（可同时调用）**：可同时调用 `use_skill("ki-memory-lookup")` 的决策记忆策略查被改模块的历史决策，**逐条**核对本次变更是否违背原决策——**不挑着看**（判断"改动是否触及某条决策"需先看全，事先不知哪条相关）；若变更**推翻**了某条历史决策，标记为需说明项，要求在变更说明中给出推翻理由（查询失败或无可用记录，忽略继续）
...
6. **范围自适应**：按变更规模选择快速/标准/深度模式；快速模式跳过专家查询、强关联查询、需求上下文感知、意图确认交互和并行派发
