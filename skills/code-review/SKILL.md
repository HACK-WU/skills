...
→ 阶段 3 功能意图推理 → 阶段 4 语义提取与建档 → 阶段 5 多维度 Review → 阶段 6 输出报告
```
...
| 🔬 深度模式（51-200行） | C0 + C1 + implementation/01-架构 + 02-实现 + 03-数据流转 | 额外辅助维度 4 架构评审、维度 2 深层 Bug 识别、维度 0.6 整体语义分析 |
...
| 🔶 大型（51~200 行） | 🔬 深度模式 | C0+C1+架构+实现+数据流转 | 分层并行 | 全部 7 维度 + 架构影响传播分析 | 默认请用户确认 | 完整报告 + 调用链影响图 |
...
**共享上下文打包**：波次1 每个子 agent 必须接收——阶段 3 功能意图（权威语义）、阶段 4 语义声明表、diff + 完整文件、受影响代码清单、专家资产（如有）、语言/运行时版本。子 agent **不重新执行**阶段 0~4，直接消费主 agent 产出，保证所有维度基于同一套语义基准。
...
| 子维度 | 检查核心 | 方向 |
|--------|----------|------|
| 0.1 提交意图 vs 实际实现 | 说修 X 真修了 X？只修表面症状？混入无关变更？ | — |
...
| 0.9 数据流守恒 | 数据从生产到消费三问：流向变了吗？数量变了吗（减少/增多）？内容还是原样吗？任一变化 → 追查所有消费方，消费方受影响即 Bug | 数据 |
| 0.10 搭车变更审计 | 把 diff 拆成「必要变更」（服务于声称意图）与「搭车变更」（顺手做的优化/清理/重构）；每条搭车变更单独立案：先过切斯特顿围栏检查（原写法是不是故意的？），再以 0.6/0.9 同等强度审查，不因"只是顺手优化"降低标准 | 范围 |
...
| 数据实体 | 流向变化 | 数量变化 | 内容变化 | 受影响消费方 | 结论 |
|----------|----------|----------|----------|--------------|------|
| `<集合/记录/字段>` | ✅ 无 / ❌ `<原流向→新流向>` | ✅ 无 / ❌ `<如 10→5，原因>` | ✅ 无 / ❌ `<字段/精度/格式变化>` | `<消费方清单及影响>` | ✅/⚠️/❌ |

**搭车变更审计（0.10，仅当 diff 含与声称意图无关的改动时）**：

| 搭车变更位置 | 改动性质 | 围栏检查（原写法是否故意） | 全局影响 | 结论 |
|--------------|----------|---------------------------|----------|------|
| `<文件:行号>` | `<顺手优化/清理/重构>` | ✅ 已证非故意（`<注释/blame/测试证据>`）/ ❓ 无法证明 | ✅ 无 / ❌ `<破坏的判据>` | ✅ 放行 / ⚠️ 建议拆分独立提交 / ❌ 回退 |
...
6. **范围自适应**：按变更规模选择快速/标准/深度模式；快速模式跳过专家查询、意图确认交互和并行派发
...
17. **搭车变更不豁免**：与声称意图无关的顺手优化/清理（0.10）不因"只是小改动"降低审查强度——它没有意图授权，反而要更严；无法证明原写法非故意设计的，不得判 ✅，最低 ⚠️ 建议拆分独立提交单独评审

## 需求管理集成

当项目配置了 `.requirements/config` 且 `storage_path` 指向有效目录时，审查完成后自动执行：

1. **获取需求上下文**：如关联了 REQ-ID，审查前读取需求信息作为参照：`req list --id {REQ-NNN} --deps`
2. **写入审查报告**到需求目录后注册文档关联：
