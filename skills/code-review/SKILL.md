...
> 0.6（纵向：局部→整体）与 0.7（横向：局部→同函数不同路径）、0.8（场景：按功能意义枚举）、0.9（数据：沿数据流追到消费方）正交互补，共同构成完整的语义影响面。
...
9. **完整语义影响面**：任何局部修改都必须做纵向（0.6 整体语义）+ 横向（0.7 路径语义）+ 场景（0.8）+ 数据（0.9 数据流守恒）四重校验，防止"局部正确、整体失真"、"此路径正确、彼路径失真"或"代码正确、数据悄悄变少"
