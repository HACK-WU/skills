...
- **专家资产辅助**：审查中等及以上变更时，按 `expert-solution-workflow` 规则查询被修改模块的相关经验、解决方案或记忆（如业务专家资产），复用已知坑、能力契约、架构知识辅助审查
...
2. 按 `expert-solution-workflow` 规则查询相关经验、解决方案或记忆（如业务专家资产包；查询失败或不可用 → 忽略，不阻塞审查）
3. 如命中专家，按审查深度分层加载：
...
**执行**：调用 [`strong-relation`](../strong-relation/SKILL.md) 的**查询模式**（提取模块关键词 → `ki_search` / `ki_query_group` 检索 → 利用结果）。ki 查询参数细节一律以 strong-relation 查询模式为准，本阶段不重复。
