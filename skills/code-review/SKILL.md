...
| 0.6 局部改动 vs 整体功能语义 | 局部正确是否破坏方法/类/模块/业务流程的整体语义？局部正确但全局失真 → P0 | 纵向 |
...
> 0.6（纵向：局部→整体）与 0.7（横向：局部→同函数不同路径）、0.8（场景：按功能意义枚举）正交互补，共同构成完整的语义影响面。
...
9. **完整语义影响面**：任何局部修改都必须做纵向（0.6 整体语义）+ 横向（0.7 路径语义）+ 场景（0.8）三重校验，防止"局部正确、整体失真"或"此路径正确、彼路径失真"
