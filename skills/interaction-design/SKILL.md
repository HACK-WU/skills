...
description: 设计人机交互层——用户怎么跟系统打交道。不画 UI，不设计技术架构，只关注交互逻辑：谁在用、怎么操作、看到什么、出错怎么办。支持多种场景类型：脚本/CLI、Web 系统、API 服务、工具/SDK 等。适用场景：(1) "设计一下这个脚本怎么用"、(2) "规划一下用户操作流程"、(3) "这个 API 的交互怎么设计"、(4) "帮我设计命令行参数"、(5) "用户遇到错误时应该看到什么"、(6) 需要在技术设计之前理清交互逻辑时。输入需求描述或需求挖掘报告，输出交互设计文档。
---
...
**目的**：设计人机交互层，理清用户与系统的交互逻辑，不涉及UI视觉和技术架构

**功能**：支持脚本/CLI、Web系统、API服务、工具/SDK等多种场景类型的交互设计，覆盖角色定义、场景梳理、流程设计、反馈状态、异常边界、交互约束
...
交互设计已完成，报告已输出。请选择后续行动：

1. 📁 落盘归档
   将交互设计文档保存到 .requirements/ 目录，便于后续引用和管理

4. 📊 进入数据流设计
   使用 data-flow-model 技能构建数据模型（ER 图）和数据流图，明确数据结构与数据流向

5. 📐 直接进入技术设计
   使用 design-craft 技能进行技术设计，跳过数据流设计阶段
...
2. **写入交互设计文档**到需求目录后，调用 `req update` 注册文档关联：
