...
description: 设计人机交互层——用户怎么跟系统打交道。不画 UI，只关注交互逻辑：谁在用、怎么操作、看到什么、出错怎么办。支持脚本/CLI、Web、API、SDK 等多场景类型。适用场景："设计一下这个脚本怎么用"、"规划用户操作流程"、"这个 API 的交互怎么设计"、"帮我设计命令行参数"、"用户遇到错误时应该看到什么"，或技术设计之前需理清交互逻辑时。
---
