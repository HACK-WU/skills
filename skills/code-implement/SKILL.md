...
- **输入**：design-to-code 产出的骨架文件 + skeleton-status.md + code-survey 调研结果 + dependency-docs 依赖文档；**或** `delivery-slicing` 产出的砖头包（并行窗口模式）
...
| 模式 | 输入 | 场景 | 差异要点 |
|------|------|------|---------|
| **顺序模式**（默认） | design-to-code 骨架 + `skeleton-status.md` | 单窗口串行实施 | 走下方阶段 0–5 完整流程 |
| **并行窗口模式** | `delivery-slicing` 砖头包（6 件套） | 跨会话 / 跨 IDE 并行，一窗一包 | 见下节，**阶段 0–2 收敛、约束更硬** |

### 并行窗口模式（冻结契约填实现）

一个窗口只负责一个砖头，**只填实现体，契约一字不改**。

**启动动作**：读砖头包 6 件（`slice.md` / `ownership.md` / `contract/` / `out-of-scope.md` / `verify/` / `handoff.md`），**不读本包以外的设计文档**。

**阶段收敛**：

| 原阶段 | 并行窗口模式下的做法 |
|--------|---------------------|
| 阶段 0 定位骨架 | → 改为**读砖头包**（路径由 `handoff.md` 给出） |
| 阶段 1 上下文构建 | → 收敛：风格 / 依赖信息应由包内或契约注释自带；**不做额外 code-survey** |
| 阶段 2 批次规划 | → 改为**读批次表**（`sub-requirements/INDEX.md`），只做本窗口这一片 |
| 阶段 3 分批编码实施 | 保留，但**只能改 `ownership.md` 的独占写路径** |
| 阶段 4 契约验证 | → 改为**跑 `verify/run.sh`**（含**本砖头组**契约测试 + **片级验收测试** + 数据走向预演 + 越界检查 + **桩残留自检**），用包内脚本而非自拟 |
| 阶段 5 落盘 | → 改为：更新 `slice.md` 状态标记 + 回报（不更新 `skeleton-status.md`） |

**硬约束（违反即拼接拒收）**：

| # | 约束 | 原因 |
|---|------|------|
| 1 | **契约只读**：`contract/` 与桩签名一字不改 | 改 = 漂移；契约只能在批次边界修订 |
| 2 | **只改独占写**：`ownership.md` 之外一律不动 | 越界 = 侵占他人独占区，拼接必然冲突 |
| 3 | **不跨砖头 import**：只能通过契约桩 / mock 交互 | 新砖头之间禁止真实代码依赖 |
| 4 | **不自造 mock**：只用骨架提供的 mock | 自造 = 验证自己想象中的对方 |
| 5 | **不扩大范围**：越界需求写阻塞上报 | 范围失控则无法判定完成 |
| 6 | **不读包外文档**：包不充分 = 包有缺陷，报缺陷而非自行补上下文 | 自包含是砖头包的质量标准 |

**分层测试期望值**（判别标准，各相关文档必须一致——**窗口只看"窗口期"那一列**）：

| 测试组 | 落盘位置 | 骨架期 | **窗口期（本砖头）** | 拼接期 |
|--------|---------|-------|-------------------|--------|
| 契约测试 · **本砖头组** | `tests/contract/{SR-XX}/` | 红 | **绿**（本窗口要让它变绿） | 绿 |
| 契约测试 · 其他组 | `tests/contract/{其他}/` | 红 | **不跑**（非本窗口职责，**不要试图让它变绿**） | 绿 |
| **片级验收测试** | `tests/{SR-XX}/` | 红 | **绿**（本窗口要让它变绿） | 绿 |
| 数据走向预演（全 mock） | `tests/data-flow/` | 绿 | 绿 | 绿（换真实实现） |
| 挂载点测试 | `tests/mount/` | 红 | **红是预期**（别的挂载位还没实现，**不要去修**） | 绿 |

> **"他组契约测试红"与"挂载点测试红"是本窗口的预期状态，不是失败。** 看到它们红就以为自己做错了、进而去改测试或改别人的桩——那是最危险的动作（制造契约漂移 + 越界——别改别人的桩）。
> **片级验收测试的断言由上游给定**（来自 5.5.1 的「承接的验收项」），**不要改判定标准**：断言不满足就去改实现，**不许改断言**——改了就是自己出题自己答。

**完成定义**：

```text
☐ verify/run.sh 全绿（本砖头组契约测试 + 片级验收测试 + 数据走向预演 + 越界检查 + **桩残留自检**）
   —— 脚本本身不跑"他组契约测试"与"挂载点测试"（它们在本窗口期本就该红）
☐ 异常路径测试已跑（失败 / 超时 / 空 / 脏数据）
☐ 片级验收测试的断言未被改动（若确实认为判据写错 → 写阻塞上报，不自行改）
☐ ★ 桩残留清零：本片生产路径无 NotImplemented / 桩标记（不依赖测试是否覆盖到）
☐ slice.md 状态标记已更新为「已完成」+ 完成日期
☐ 无未上报的越界需求
```

> **桩残留为何单列一条**：桩没替换完 = 该接口调用即抛异常。**契约测试恰好没覆盖到就抓不住**（它只跑有测例的接口），所以它是独立检查项，不是"测试绿了就等于做了"。

**阻塞上报格式**（写入 `slice.md` 状态区，继续做不受影响的部分）：
...
**契约不够用时的唯一合法路径**：不要就地改 → 写阻塞 → 当前批次结束 → 回 `design-craft` 改骨架 → 更新契约快照与基线 commit → 下一批开工。

**若问题不是"契约细节"而是"边界本身"**（如发现两片必须共享同一段可变逻辑、或某片根本 mock 不出依赖方）→ 那属于 **划分修订**，比契约修订贵一个量级（骨架全废重出）。**此时必须停下写阻塞报用户**，不许悄悄合并两片或自行扩大独占写——那会让其他窗口的边界约定同时失效（判定信号见 `delivery-slicing` 阶段 0「两级修订」）。

> 参数化细节（六件套模板、语义契约 10 项、越界检查脚本）见 `delivery-slicing` skill 的 [reference.md](../delivery-slicing/reference.md)。
...
> **两种"并行"别混淆**：本节的并行是**同会话内**由主 agent 派子 agent（`task-dispatch`，有主 agent 仲裁、靠 `send_message` 协调）。**跨会话 / 跨 IDE 的手工并行**走「并行窗口模式」——没有主 agent、Agent 之间不能通信，协作靠砖头包与文件，不靠消息。
...
- 并行任务调度：`task-dispatch` skill（**同会话内**顺序无关批次并行编码）
- 跨会话并行交付（砖头包 / 契约快照 / ownership）：`delivery-slicing` skill
- 统一拼接（跨窗口产出合流）：`integration-merge` skill
