...
> **数据守恒三问**（每条数据流逐跳检查）：①流向——数据去向/消费方与设计一致吗？②数量——到达消费方时有无静默增减（如过滤/异常丢弃/重复消费）？③内容——字段/精度/格式到达消费方时还是原样吗？任一变化且消费方受影响 → 记 ❌ 并归入问题汇总的「数据问题」。
...
| **数据问题** | 数据流转或约束存在问题（含守恒破坏：流向改变/数量静默增减/内容变形） | 修改数据流图或设计 |
