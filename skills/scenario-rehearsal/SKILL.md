...
description: 通过模拟真实使用场景验证可行性。支持四种对象：设计文档（验证设计点可行性）、需求文档（验证需求完整性/一致性/验收可达性）、Skill（验证触发准确性、指令完整性等质量维度）和代码（纸上执行：用具体场景实例沿代码追踪状态演进，验证终态与中间态）。适用场景："推演一下这个设计"、"验证设计方案"、"推演一下这个需求"、"推演一下这个 skill"、"推演一下这段代码"、"模拟跑一下这个函数"、"桌面走查"，或 design-craft 完成后自动触发。
---
...
**目的**：在技术设计完成后、开发之前，或 Skill 编写后、发布前，或代码编写后、运行之前，通过模拟真实使用场景验证设计/Skill/代码的可行性
...
- 代码模式：代码文件/函数/diff +（可选）关联需求或设计文档（作为预期终态的依据）
...
**代码模式**：
- 代码编写或修改完成后，想知道"某个具体场景在这段代码上走下来会发生什么"
- 静态审查（code-review / challenger）之外，需要沿时间轴正向模拟执行、追踪状态演进时
- 典型时机：编码完成后按需触发；审查报告存在 `[需运行验证]` 存疑项，想先低成本把存疑点具体化为场景实例时
...
- 代码模式："推演一下这段代码"、"模拟跑一下这个函数"、"这个输入进去会怎样"、"桌面走查"、"trace 一下"、"code rehearsal"
...
- 代码模式：已有可读的代码（文件/函数/diff）；关联需求/设计文档可选（用于确定预期终态）

## 核心原则

1. **角色驱动**：以执行者为入口，每个角色独立推演
2. **双重验证**：既要验证数据走向，也要验证关键设计点实现（设计模式）；验证 Skill 质量（Skill 模式）；验证执行轨迹与终态（代码模式）
...
7. **可视化输出**：核心场景推演路径默认用 🕸 文字树（渲染无关），协作时序与问题分布按需用 mermaid 增强，问题一目了然
...
```
阶段 0：对象类型识别 + 全局理解 → 判定推演对象：设计文档 / 需求文档 / Skill / 代码；先立全局图景再拆场景
阶段 1：角色识别         → 按对象类型从设计文档/需求文档/Skill/代码提取所有执行者角色
阶段 2：场景提取 + 策略识别 → 按对象类型提取场景；设计模式多选启用 profile，需求模式用需求质量维度，Skill 模式用 Skill 质量维度，代码模式用场景实例化 + 代码推演维度
阶段 3：推演执行         → 按角色走完整流程，按对象类型验证设计点/需求质量/Skill 质量，双重验证；每场景结论回扣全局目标
阶段 4：问题汇总         → 按严重度分级，按类型分类
阶段 5：结果展示         → 直接展示推演结果（含 🕸 文字树，mermaid 可选增强），与需求管理集成
```
...
| **代码** | 输入为代码文件/函数/diff，用户要求推演/模拟执行/走查代码 | 走代码模式：阶段 1~3 对场景实例做纸上执行，使用 [strategies/code.md](strategies/code.md) 维度 |

**四种模式的本质区别**：

| 维度 | 设计文档模式 | 需求文档模式 | Skill 模式 | 代码模式 |
|------|------------|------------|-----------|---------|
| 验证目标 | 设计点在真实场景中是否可行 | 需求是否自洽、无缺口、验收可达 | Skill 触发准确性、指令完整性、示例质量、工作流合理性 | 代码在具体场景实例下的实际行为是否符合预期终态 |
| 数据走向 | 验证数据从创建到消费的链路 | 仅当需求描述了数据流时验证；否则跳过 | 不验证数据走向；验证 Skill 引用的资源完整性 | 验证运行时状态演进（变量/资源/副作用轨迹） |
| 验证维度 | 设计点维度（3.2）+ 设计 profile 加压 | 需求质量维度（见 strategies/requirement.md） | Skill 质量维度（见 strategies/skill.md） | 代码推演维度（见 strategies/code.md） |
| 触发时机 | design-craft 之后 | requirement-mining 之后、design-craft 之前 | create-skill 之后、skill 更新后 | 编码完成后按需触发（不自动接入审查链） |
| 执行者 | 用户角色 + 自动化程序 | 用户角色 | AI 系统 | 调用方角色 + 环境角色 |

> 若用户同时给了需求文档和设计文档，默认走**设计文档模式**（设计已存在，需求推演价值已部分被设计吸收）；如用户明确要"先推演需求"，则切需求模式。
> 若用户明确要求推演 Skill（输入含 SKILL.md），则走 Skill 模式，无论是否还有其他文档。
> 若用户明确要求推演代码（输入为代码/diff），则走代码模式；随附的需求/设计文档仅用作预期终态依据，不切换模式。
...
**代码模式辅助——强关联关系查询**：理解"系统位置"（上游/下游依赖）时，若推演对象是代码，可调用 `use_skill("ki-memory-lookup")` 检索相关模块的强关联关系（跨模块契约/业务耦合），作为系统位置/模块间牵动的补充依据（查询失败或无可用记录，忽略继续）。
...
| **调用方角色**（代码模式） | 触发这段代码执行的主体 | 用户请求、定时任务、上游服务、并发线程 |
| **环境角色**（代码模式） | 会"出牌"的外部依赖（代码模式特色，review 不把依赖当角色） | 网络会超时、磁盘会满、第三方会返回乱序/错误码、时钟会回拨 |
...
**代码模式**：
1. **调用方角色**：从函数签名/调用点/入口推断谁会触发这段代码（同步调用方/定时触发/并发调用者），不同调用方携带不同输入域与时序
2. **环境角色**：识别代码依赖的外部实体（网络/磁盘/DB/第三方 API/时钟/锁），并假设它们会以失败/慢/乱序的方式"出牌"——这是代码推演区别于静态审查的核心
3. **从代码提取**：通读目标代码，识别入口参数、分支条件、外部调用点、共享状态，作为阶段 2 场景实例化的原料
...
- 代码模式：**场景实例化**（与其他模式的本质区别）——不是抽象的"异常场景"，而是带具体值的可执行实例（如 `余额=0.01，重试=3 次，网络在第 2 跳超时`）；三个来源：① 输入域切分（正常/边界/非法值）② 环境状态（依赖失败/慢/乱序）③ 并发时序（指定具体交错：线程 A 在 T1 读、线程 B 在 T2 写）；**每个实例推演前先声明预期终态**（防止推演完后合理化结果）
...
**代码模式 — 推演验证点提取**：不提取设计点/质量点，而是以阶段 2.1 的**场景实例**为单位，每个实例声明：具体输入值 + 初始状态 + 环境行为（依赖是否失败/慢/乱序）+ 并发交错（如有）+ **预期终态**。验证维度见 [strategies/code.md](strategies/code.md)。
...
>
> **代码模式不启用上述设计 profile**。此时跳过本表扫描，统一使用 [strategies/code.md](strategies/code.md) 的**代码推演维度全集**（终态正确性 / 中间态安全 / 时序鲁棒性 / 资源生命周期 / 可观测性），进入阶段 3 的纸上执行。代码模式的推演矩阵为「场景实例 × 推演维度」（而非设计点 × 场景）。
...
**代码模式**：不验证静态数据走向，而是做**纸上执行（desk check / trace）**——逐步追踪每个场景实例的运行时状态演进，产出**执行轨迹表**：

```text
| 步骤 | 代码位置 | 关键状态变化 | 副作用 |
|------|---------|-------------|--------|
| 1 | order.py:42 | balance: 0.01 → -9.99 | 无 |
| 2 | order.py:45 | ❌ 负数未拦截，进入扣款 | 调用支付 API |
```

> 轨迹的**主干**也可用 🕸 文字树表达（状态演化挂节点：`余额 0.01 → -9.99`，问题点标 `❌`，见 5.3）；**表格保留**，作为副作用 / 中间态字段的明细——树看主干，表看字段。

> 每个实例追踪完后做**终态对比**：预期终态 vs 推演终态，不一致即为偏差。除终态外，每一步后追问：若此步失败/进程在此崩溃，已发生的副作用留下的中间态是否一致、可恢复（中间态安全，代码模式最擅长抓的“半途而废留脏数据”）。
...
**代码模式 — 验证维度**（不验证设计点/需求质量/Skill 质量）：使用 [strategies/code.md](strategies/code.md) 的代码推演维度全集——终态正确性🔴 / 中间态安全🔴 / 时序鲁棒性🔴 / 资源生命周期🟡 / 可观测性🟡，各维度的验证问题与轨迹表格式见策略文件。

> 代码模式不跑设计 profile 加压，沿时间轴对每个场景实例逐步纸上执行，逐维度判定。
...
>
> **代码模式**：本节改为「执行轨迹表 + 终态对比」，逐步追踪代码位置/状态变化/副作用，末尾给出预期终态 vs 推演终态的对比判定（格式见 strategies/code.md），不验证设计点。
...
| **终态偏差**（代码模式） | 推演终态与预期终态不一致 | 修正代码逻辑，使终态符合预期 |
| **中间态泄漏**（代码模式） | 某步失败/中断后留下不一致或不可恢复的脏状态 | 补充事务/回滚/补偿，保证中间态一致 |
| **时序缺陷**（代码模式） | 特定并发交错或外部乱序下行为错误 | 补充同步/幂等/顺序保证 |
| **资源泄漏**（代码模式） | 异常路径下连接/句柄/锁未释放 | 补充 finally/defer/context 释放 |
...
> 推演时间：YYYY-MM-DD
> 推演对象：{设计文档模式 / 需求文档模式 / Skill 模式 / 代码模式}
> 输入文档：{设计文档 / 需求文档 / SKILL.md / 代码文件或 diff 路径}
...
## 3. 场景推演详情
（阶段 3 输出，按场景分节；核心场景含 🕸 推演路径文字树，协作场景可选 mermaid 时序图，见 5.3）

## 4. 问题汇总
（阶段 4 输出）

## 5. 推演结论（可选 mermaid 问题分布饼图，见 5.3）
...
### 5.3 可视化输出规范（🕸 文字树默认 + mermaid 可选增强）

为提升展示直观性，**核心场景推演路径默认用 🕸 文字树**（渲染无关：聊天 / 终端 / Word / 飞书都可读），mermaid 作为渲染环境支持时的可选增强：

| 表达 | 适用场景 | 要求 |
|------|---------|------|
| 🕸 **文字树**（默认） | 核心场景推演路径 / 代码模式执行轨迹 | 每个核心场景一棵：节点=步骤，用 ✅/⚠️/❌ 标注验证结果，问题节点标 `← ❌ 问题` |
| `sequenceDiagram` | 协作场景（多角色按时间序） | 可选增强——**多角色交互时推荐**（渲染环境支持就画）：参与者=角色，消息=步骤，标注验证结果 |
| `flowchart TD` | 核心场景推演路径（细粒度分支） | 可选增强：节点=步骤，✅/⚠️/❌ 标注，问题节点高亮 |
| `pie` | 问题分布 | 可选：结论处一张：🔴阻断/🟡警告/🟢建议占比 |
| `flowchart LR` | 有问题的数据走向分支 | 仅当数据走向验证发现问题时画，不画全量（避免与 data-flow-model 重复） |

**🕸 文字树最小语法（跨 skill 统一，4 条）**：① `│`+`▼` 串联向下、`├─` `└─` 分支 / 并行；② 节点挂数字与状态（值演化 / 耗时），关键节点打标记（`← ❌ 问题` / `← 待验证`）；③ 并行标"（取 max）"、串行标"（串行相加）"；④ 只画结构类信息，清单仍用表格；节点 ≤15、层级 ≤4。

> 各 `strategies/*.md` 的「mermaid 提示」**一律按"可选增强"理解**（默认先用 🕸 文字树，渲染环境支持且图更有表现力时才画 mermaid），无需逐文件改写；`strategies/code.md` 的执行轨迹已改为文字树默认。
...
- ❌ **设计/需求/Skill 模式下关注代码实现细节**：这三种模式推演关注设计/需求/Skill 层面，不下沉到具体代码实现；**代码模式则相反**，以代码为推演对象、以执行轨迹为产出
- ❌ **超出推演对象范围**：不评审当前推演对象（设计/需求/Skill/代码）范围外的问题
...
- ❌ **代码模式退化为静态审查**：代码模式必须基于具体场景实例沿时间轴正向模拟执行（追踪状态演进），而非像 code-review 那样逐维度静态扫描、也非像 challenger 那样对已有结论做对抗质疑；无具体输入值/初始状态的抽象推演无效
- ❌ **编造虚构的执行轨迹**（代码模式）：面对深调用栈/复杂三方库/不可静态确定的行为，禁止编造看似精确实则臆想的状态值（用精确外观骗取信任比模糊结论更危险）；跨越不确定处必须标注 `[需运行验证]`，超出可靠追踪范围则降级为"推演到此为止 + 待验证清单"
- ❌ **推演前不声明预期终态**（代码模式）：先跑后定结论会导致合理化推演结果，必须先声明预期终态再追踪

## 附加资源

- 推演示例 / 关键设计点验证维度详解：[reference.md](reference.md)
- 推演策略 profile：[strategies/](strategies/)（设计模式按设计特征多选启用，每个 profile 含精选维度/加压点/典型异常/mermaid 提示；需求模式使用 [strategies/requirement.md](strategies/requirement.md) 的需求质量维度；Skill 模式使用 [strategies/skill.md](strategies/skill.md) 的 Skill 质量维度；代码模式使用 [strategies/code.md](strategies/code.md) 的代码推演维度）
