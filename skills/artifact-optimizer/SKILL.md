...
- **专家资产复用**：分析前按 `expert-solution-workflow` 规则查询相关经验、解决方案或记忆，复用已知坑和最佳实践
...
3. **专家团查询**：按 `expert-solution-workflow` 规则查询相关模块的经验、解决方案或记忆（如业务专家资产包；查询失败或不可用则忽略此步骤继续正常流程）。如有相关资产，加载已知坑和最佳实践作为分析参考
