...
description: 对整体功能或完整模块做交付前验收：以用户确认过的验收指标清单逐条核验（已有测试套件 + 临时场景测试 + e2e 真实链路三层证据），只判定不改产品代码，产出锚定分支与 commit 的验收报告，支持复验。仅用户明确指定时触发（"验收这个功能/模块"、"做验收"、"验收测试 / acceptance test"、"复验"）；只问"流程跑通没有"用 e2e-testing。
disable: false
---
...
- （**被动调用**）并行交付中由 `integration-merge` 阶段 0.5 调用做**片级验收** → 见下方「片级验收模式」；该用法不由用户发起，也不进 L3 流程
...
## 片级验收模式（被 `integration-merge` 阶段 0.5 调用）

本技能有两种用法，**别混淆**：

| 模式 | 触发 | 对象 | 成本 | 走什么流程 |
|------|------|------|------|-----------|
| **整体验收**（默认） | **仅用户明确指定** | 一个完整功能 / 模块 | 高（起容器、跑 L3 真实链路） | 下方完整阶段 0–7 |
| **片级验收**（本模式） | 被 `integration-merge` **阶段 0.5** 调用 | 一个**砖头包**（并行交付单元） | 低（不起容器） | 只走下方「三步」 |

### 三步

1. **锁定验收项**：读该片 `slice.md` 的验收标准表——它由上游给定（`design-craft` 5.5.1 承接的验收项 → 5.5.2 可执行化），**不是实现窗口自拟**；执行中不得改判据
2. **逐条核验**：跑对应命令、对照期望输出；**每条执行前先写下预期结果**（跑完再定结论会让一切"通过"）
3. **只判定不修改**：不达标 → **退回该片**（由实现窗口修），验收方不动手

### 本模式**不做**的事（与整体验收的差异）

| 不做 | 为什么 |
|------|--------|
| 不推导指标 | 验收项已由 `design-craft` 5.5.1/5.5.2 产出（避免与上游双源） |
| **不起容器、不跑 L3 真实链路** | 片级只证"片内功能 + 形状"；**跨片效果归拼接级**（`integration-merge` 阶段 4） |
| 不在此现写场景测试 | 验收测试骨架已在骨架阶段生成（红 → 绿），窗口已让它变绿 |
| 不由本技能自行触发 | 只在 `integration-merge` 调用时进入；用户仍可随时按**默认模式**发起整体验收 |

### 仍适用的铁律（四条）

- **指标先确认后执行**：验收项锁定后不得边跑边改（测不出来标 `⚪`，不删项、不放宽）
- **断言锚定指标、不锚定实现**：不读实现再反推断言
- **只判定不修改**：不修产品代码
- **⚪ 不等于 ✅**：未验项单列「需补验清单」

### 降级

无独立验收方（单人单机）时，至少做到「验收项外部给定（**非窗口自拟**）」+ 用户抽查，并在报告中**显式声明"未独立验收"**。

> **交叉引用**：片级验收在拼接流程中的位置见 `integration-merge` 阶段 0.5（它是拼接前的门，与阶段 1「产出预检」分工：**0.5 管做没做对，1 管规不规范**）。
