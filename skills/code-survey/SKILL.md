---
name: code-survey
description: 在设计前对代码库进行按需调研，只调研与当前需求相关的维度（代码风格、路径、架构、类似功能、技术栈、数据存储、错误处理、安全、日志、API、测试、配置、部署）。强制 ki 记忆索引优先（项目记忆 → 知识库记忆），不足时再代码搜索。需搜索的维度 ≥ 2 时自动调用 task-dispatch 并行搜索以加速。适用于"代码调研"、"了解现有代码"、"项目代码风格"、"代码路径"、"架构调研"等场景，或在 design-craft 等技能的设计前置阶段被调用。
---
...
**功能**：按需调研代码库的 13 个维度，根据需求特征自动筛选需要调研的维度，强制 ki 记忆索引优先获取信息，代码搜索阶段 ≥ 2 个维度时自动调用 `task-dispatch` 并行搜索加速。
...
```
ki 项目记忆索引 → ki 知识库记忆索引 → 代码搜索（并行） → 语义检索兜底
   (ki_query_group)     (ki_query_group)   (task-dispatch)    (ki_search)
```

### 第 0 步：检查 ki 可用性

**必须首先执行。** 若 `ki_query_group` 工具不可用 → **中断调研**，提示用户安装 ki MCP 服务。

### 第 1 步：查询项目记忆索引

使用 `ki_query_group(mode="hot", scope="${scope}-memory")`：

- **若命中**：直接复用记忆内容，跳过对应维度的代码搜索，标注来源「项目记忆索引」
- **若未命中**：记录结果，进入第 2 步

### 第 2 步：查询知识库记忆索引

使用 `ki_query_group(mode="hot", scope="${scope}")`：

- **若命中**：提取相关模块信息，跳过对应维度的代码搜索，标注来源「知识库记忆索引」
- **若未命中**：记录结果，进入第 3 步
...
**仅当前 3 步均无法覆盖时使用**：`ki_search(scope="${scope}")`
...
- **文档要求**：
  - 仅记录实际调研过的维度
  - 每个维度标注信息来源（项目记忆索引 / 知识库记忆索引 / 代码搜索 / 语义检索）
  - 内容简略，重点是帮助设计决策
