---
name: design-to-code
description: 从设计文档生成代码骨架（文件 + 类 + 方法），把接口契约、数据模型、异常处理写进契约级注释，让后续编码不必反复翻文档；生成前先与用户确认架构风格与设计模式。触发："生成代码骨架"、"搭骨架"、"按设计生成代码结构"，或设计完成后准备编码。
---
...
- **输入**：design-craft 产出的设计文档目录（`design/DESIGN.md` + 子文档，也兼容单文档快速通道）；走并行路线时另加 **design-craft 阶段 5.5.1 的模块划分**（决定桩与挂载点的落点）与 **5.5.2 的骨架交付清单**（11 项，含数据走向预演链路场景 / 横切约定 + 非功能预算 / 权限模型）
- **输出**：项目源码目录中的代码骨架文件（文件已创建、类已定义、方法已声明、契约已注释，方法体用语言原生占位符）

### 并行路线的骨架差异（design-craft 阶段 5.5 判定为"走并行链"时）

默认骨架是"给单个窗口顺序填充用"；走多窗口并行时，骨架还要承担**清空共享面**的职责，需额外满足：

| 差异项 | 顺序路线 | 并行路线 |
|--------|---------|---------|
| 方法体占位 | 语言原生占位符 | **抛 `NotImplemented`**（保证主干编译通过且调用即失败，不静默返回假值） |
| **桩标记（★ 供桩残留扫描）** | 不需要 | 桩体必须注入**自定义统一标记**（形如 `STUB:{SR-XX}:{方法名}`），并把**扫描模式写入单一来源 `.delivery/stub-pattern`**（一行正则，如 `STUB:`）——`verify/run.sh` 与 `premerge-check.sh` 都从它读，**避免三处各写一份而漂移**。**不要用 `NotImplemented` 当扫描模式**：抽象基类 / 协议声明等语言惯用法也用它 → 大面积误报 → 狼来了导致真残留被忽略 |
| 契约注释 | 参数 / 返回值 / 异常 / 预期行为 | **追加语义契约 10 项**（前置 / 后置 / 空值 / 错误 / 单位精度 / 时间 / 幂等 / 并发 / 事务 / 副作用） |
| 挂载点 | 按需 | **在现有大文件预留挂载位**（含挂载顺序） |
| 全局清单 / 数据模型 / 公共代码 / **横切约定 + 非功能预算** / **权限模型** | 设计文档说明即可 | **落成代码或配置并声明只读**（之后只增不改）；后两类见 5.5.2 第 10 / 11 项，**最易漏且不冲突不报错**。横切约定的典型落点：统一 logger formatter / trace 中间件 / 精度与时区常量 / ID 生成器 / 分页封装 / **错误码号段常量表**（按砖头分段）——落在公共代码区，即第 6 项冻结范围 |
| mock | 无 | **两类一并生成**：① 跨砖头依赖 mock（给调用方用）② **桩的 mock 版**（给数据走向预演填满全链路用）。含失败 / 超时 / 空 / 脏数据异常路径 |
| **数据走向预演测试** | 无 | **一并生成**（按 `design-craft` 阶段 5.5.2 第 8 项给的链路场景，逐跳断言；**必须用 mock 填满全链路**）；落 `tests/data-flow/`，**不按砖头分组**（它天然跨砖头，测的是链路不是单个砖头） |
| **片级验收测试骨架** | 无 | 按 `design-craft` 阶段 5.5.2 第 9 项的验收清单生成；落 `tests/{SR-XX}/`，**断言由验收项派生（不是照着实现写）**，骨架期应为红 |
| **测试与脚手架的落点（★ 分开）** | 按需 | **产品资产**（四类测试）→ `tests/` 下；**脚手架**（mock）→ `.delivery/mocks/`。判据：交付上线后还会因产品行为变化而失败 → 提交；否则 → 忽略（完整约定见 `delivery-slicing`「测试与脚手架的归属」） |
| 验收 | 骨架可编译 | 骨架可编译 **+ 分层测试达其骨架期期望**（见下）+ **桩标记全量注入（供 ⑥ 号拒收门扫描）** |

> ### ★ 桩残留扫描的三个易错点（都属「会静默出错」）
>
> | # | 易错 | 后果 |
> |---|------|------|
> | 1 | **扫描对象用 `git grep`** | 它只扫**已跟踪**文件，而实现期的桩正是**新建未提交**的 → 扫描**恒为空** → **假绿**（**比不扫更危险**：它给出"已检查且通过"的错误结论）→ 必须扫**工作区文件系统**（`grep -rn`） |
> | 2 | **用语言原生占位符当模式**（如 `NotImplemented`） | 抽象方法 / 协议声明等**语言惯用法**也用它 → 大面积误报 → 狼来了 → **真残留被忽略** |
> | 3 | **契约 / 文档文件里写了完整标记字面量** | 它会**命中自己**（实测：契约注释里写 `STUB:SR-01:*` → 必然误报）→ 契约里只写"标记模式见 `.delivery/stub-pattern`" |
>
> **扫不到的代价**：漏实现的接口调用即抛异常，而**契约测试只覆盖有测例的接口** ——
> 漏实现的恰是**没被覆盖**的那些。所以它是**独立机械扫描**，不读测试结论。

**分层测试期望值**（全流程判别标准，各相关文档必须一致）：

| 测试组 | 落盘位置 | 骨架期 | 窗口期（本砖头） | 拼接期 | 验什么 |
|--------|---------|-------|----------------|--------|--------|
| 契约测试 · **本砖头组** | `tests/contract/{SR-XX}/` | 红 | **绿** | 绿 | 本砖头是否兑现契约（形状） |
| 契约测试 · 其他组 | `tests/contract/{其他}/` | 红 | **不跑**（非本窗口职责） | 绿 | — |
| **片级验收测试** | `tests/{SR-XX}/` | 红 | **绿** | 绿 | **本砖头是否达成需求验收项**（功能） |
| 数据走向预演（**全 mock**） | `tests/data-flow/` | **绿** | 绿 | 绿（换真实实现） | 断言链是否完整可执行 |
| 挂载点测试 | `tests/mount/` | 红 | 红（**预期**，别的挂载位还没实现） | 绿 | 装配是否接通 |

> **契约测试与片级验收测试的区别**：前者验"接口是否兑现契约"（形状，由 `sub-requirements/{SR-XX}/contract/` 的契约快照派生）；后者验"**需求意图是否达成**"（功能，由 `5.5.1 承接的验收项` 派生）。**两者都必须由骨架阶段生成（红），窗口只负责让它们变绿**——窗口自己写验收断言 = 自己出题自己答。

**契约测试必须按砖头分组**：

- 分组口径 = **被测接口的实现方**——`tests/contract/SR-01/` 放"SR-01 所提供接口"的契约测试
- **为什么必须分组**：不分组的全局契约测试，在窗口分支上永远无法变绿（别人的桩还没实现）→ 窗口的完成定义不可达 → 窗口最可能的动作是**改测试来凑绿**，反而制造契约漂移
- 分组后每个窗口只对本组负责，完成定义可达

> **「红是健康，绿是警报」**：骨架期契约测试若出现 `passed`，说明桩返回了看似合法的默认值（`None` / `[]`）——那是**假绿**，拼接期也不会暴露，必须归零。
> **数据走向预演必须用 mock 填满全链路**：用 `NotImplemented` 桩会跑到第二跳就抛异常，后面的贯通断言根本执行不到。
> 数据走向预演红了 = **契约链有缺口**（缺字段 / 单位不一致 / 时区不一致）——此时改零成本，**不要带缺进并行**。

> 骨架核验与预演的 SSOT 在 `delivery-slicing`；生成完成后即交由它做 **阶段 0 骨架完备性核验**（存在性）与 **阶段 0.5 骨架预演**（可执行验证：零上下文预演 / 分层测试期望值 / 数据走向预演 / 覆盖反向 / 独立核验方）。
...
- 并行任务调度：`task-dispatch` skill（**同会话内**同批顺序无关时自动调用）
- 跨会话并行交付（骨架核验 + 切片 + 批次）：`delivery-slicing` skill
