---
name: design-to-code
description: 从设计文档生成代码骨架（文件+类+方法结构），并将设计文档中的接口契约、数据模型、异常处理等以契约级注释的形式就地嵌入到代码中，使 AI 编码时无需反复查阅外部文档，直接看代码注释即可补全实现。大型项目按依赖批次分批生成骨架，同批顺序无关时自动调用 task-dispatch 并行加速。适用场景：(1) 设计文档完成后准备编码，(2) "生成代码骨架"、"搭骨架"、"按设计生成代码结构"、"骨架先行"，(3) 设计变更后更新骨架，(4) AI 编码阶段需要设计意图就地化以降低 token 消耗，(5) 修改现有代码时在变更点标注 TODO + 契约注释。输入设计文档目录，输出代码骨架文件组（可直接被 IDE 解析）。**在正式生成骨架之前，会先评估当前设计适用的架构风格与设计模式，以改进可读性、扩展性、可维护性等质量属性，并与用户确认所选方案后才开始编写骨架。**
---
