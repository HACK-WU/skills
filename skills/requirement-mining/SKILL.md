...
**需求分析前置——强关联关系查询**：分析涉及既有模块的需求时，可调用 `use_skill("ki-memory-lookup")` 的强关联策略，检索相关模块的强关联关系（跨模块契约/业务耦合），识别"改动会牵动哪些其他模块"，作为需求影响范围与依赖分析的补充依据（查询失败或无可用记录，忽略继续）。

**需求分析前置——决策记忆查询**：分析需求时，可调用 `use_skill("ki-memory-lookup")` 的决策记忆策略，检索相关模块是否已有历史决策记录——若分析得出的新建议恰是已被否决的方案，先核对原否决理由再重新提出，避免把已被否决的方案当新建议重复建议（查询失败或无可用记录，忽略继续）。
...
> 站点是否"多页文档站"拿不准时，用 `web-index` 的抓取脚本试抓一次（成本极低）即可判定，不必靠猜。

**为什么要分出"登记待建"**：需求挖掘阶段方案还没定，此时花一轮上下文把整个文档站的链接收集下来，需求一旦转向就全白费——**先登记，等确定要用再建**。

输出并入 5.5.2 的结论与报告 §4.9：

```text
外部依赖文档可查阅性：
- Stripe API 文档：多页文档站，需求已确认且后续要反复参考 → 现在建索引（slug 由 `web-index` 生成）
- Twilio 文档：本次只需确认能否发短信，查一次 → 直接查，已确认可用
- OpenAI Assistants 文档：方案未定 → 登记待建，设计阶段再建
```
...
**统一确认模板**（6.5.1 与 6.5.3 共用，按场景选对应检查项）：
```markdown
## {标题}确认
...
- **可快速实现（低复杂度）** → 先调用 use_skill("expert-solution-workflow") 查询相关经验、解决方案或记忆复用已有资产，再直接进入编码实现，跳过设计阶段
...
- `assets/` 图形目录（如有）**纳入索引**——图是需求报告的组成部分，丢了 §2.10 就是空白
...
**可快速实现（低复杂度）**：
1. 先调用 use_skill("expert-solution-workflow") 查询相关经验、解决方案或记忆（如业务专家资产包；查询失败或不可用，忽略此步骤继续正常流程）
...
3. 写入文档：使用 `write_to_file` 按 frontmatter 模板写入 `requirement.md`；**如报告含 SVG 独立图形**，先写入需求目录 `assets/`（kebab-case 命名），md 内用 `![说明](./assets/xxx.svg)` 相对路径引用，并同步进 `index.md`——SVG 是 `requirement.md` 的附属资源，**不注册进 `docs`**（文档类型枚举无 diagram，硬塞会污染 meta.json）
...
### 2.8 非功能性需求
### 2.9 关键假设与验证建议
### 2.10 现状与预期（图示；命中变更面时必填；Step 5 判定情况 B/C 时为重绘后版本）
- 变更面：[执行流程 / 数据流 / UI / 状态流转 / 无形状变化]
- 图示：[预期图（必画）+ 现状图（现状不可知时省略并注明"现状待确认"），或单图差分标注 / ASCII 双屏]
- 现状来源：[用户提供 / 读代码确认 / ⚠️ AI 推断（对应 §2.9 假设）]
- 变更点表：[变更点 | 现状 | 预期 | 类型 | 关联假设]
...
- 潜在副作用
### 3.5 建议（短期/长期/如坚持原方案）
### 3.6 关键决策点（决策轨迹留痕；收敛方案评估与"如坚持原方案"的决策信息；不写 ki，落地后由 expert-team 收割）
...
### 4.9 可行性分析
- 可行性结论：[可行 / 条件可行 / 不可行]
- 外部依赖文档：[直接查 / 消费已有索引 / 现在建索引（站点与范围） / 登记待建（站点清单）]
- 落地障碍：[列出关键技术/依赖/资源障碍]
- 前置条件：[需满足的条件与规避方案]
- 可行性约束：**条件可行/不可行**时，转译的需求清单需标注可行性约束（如"依赖 XX 前置条件""存在技术风险需先验证"）
...
- **图形化表达**：涉及流程/数据流/UI/状态变化的需求，必须在**分析阶段（Step 3）**给出「现状 → 预期」图示对比，让用户最快发现理解偏差；未命中变更面的需求（纯性能/兼容性/文案/权限）不画图
- **现状不臆造**：图示的"现状"部分必须标注来源（用户提供 / 读代码确认 / ⚠️ AI 推断），推断项须挂到关键假设 ID 并请用户核对；无法确认时画骨架 + `[待确认]`，不得凭常识补全
- **图不越界**：需求阶段只画效果层变化（步骤、流向、界面、状态），不画技术架构、表结构、接口时序——属 `design-craft` / `data-flow-model` / `interaction-design` 职责；本 skill 的图是需求侧预期草图，不替代正式设计图
...
- **可行性分析**：方案方向确认后、需求转译前，必须进行需求可行性分析，评估技术/依赖/资源可落地性，给出可行/条件可行/不可行结论。与复杂度评估区分：可行性回答"能否落地、值不值得"，复杂度回答"工作量多大、能否快速实现"；可行性高时简要带过，异常时才展开确认
...
- **外部依赖文档按规模处置**（Step 5.5.3）：需求阶段**默认不建索引**——只做可行性判定时登记待建、查一次就够的直接查、仅当"需求已确认 + 后续会反复参考"才现在建。判定规则 SSOT 在 `web-index`，本 skill 不复制规则，只写需求阶段的落地口径
- **快速实现前查资产**：当复杂度评估为低复杂度、推荐直接编码实现时，必须先调用 use_skill("expert-solution-workflow") 查询相关经验、解决方案或记忆（如业务专家资产包）。找到则加载相关资产（契约层、已知坑、实现导航等）以专家视角编码；未找到则正常编码实现
