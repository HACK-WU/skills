...
description: 从用户的功能描述中深度挖掘真实需求，打穿表象找到根因，并将产品需求转译为可落地的技术需求清单。适用场景：(1) 用户提出一个产品功能描述或改善诉求时，(2) 用户说"我想做一个xxx功能"、"能不能支持xxx"、"需要一个xxx的能力"时，(3) 用户抱怨/吐槽"这个太难用了"、"总是出问题"时，(4) 用户反馈性能或体验问题"太慢了"、"经常报错"时，(5) 用户探索性提问"看看有什么优化空间"、"分析一下这个系统"时，(6) 需要从原始需求出发理清背后真正动机和场景时，(7) 需要将产品需求拆分为技术需求时。采用"先推理后确认"模式，主动分析而非被动提问。
---
...
- 快速实现时自动查询专家团（`expert-lookup`），复用已有业务专家资产加速编码
- 维护需求目录索引文件，自动归档过时文档
...
- **可快速实现（低复杂度）** → 先查询专家团（`expert-lookup`）复用已有资产，再直接进入编码实现，跳过设计阶段
- **不可快速实现** → 进入特征匹配，推荐最合适的下一步

> **流程衔接**：本步骤输出推荐行动后，用户可选择"按推荐行动执行"（直接执行，跳过 Step 8）或"选择其他后续行动"（进入 Step 8 从完整选项列表中选择）。
...
请确认后续方向：
1. [按推荐行动执行] — 直接执行推荐行动，跳过 Step 8 选择
2. [选择其他后续行动] — 进入 Step 8，从完整选项列表中选择
```
...
5. 📊 进入数据流设计
   使用 data-flow-model 技能构建数据模型（ER 图）和数据流图，明确数据结构与数据流向

6. 📐 直接进入技术设计
   使用 design-craft 技能进行技术设计，跳过交互设计和数据流设计阶段

7. 🧪 进入验证原型
   使用 demo-verify 技能构建验证原型，确认技术可行性后再投入开发
...
**可快速实现（低复杂度）**：
1. 先调用 `expert-lookup` 技能查找是否有相关的业务专家资产包（如调用失败或不可用，忽略此步骤继续正常流程）
...
**不可快速实现（中/高复杂度）**：
> 当前需求复杂度较高，不建议跳过设计阶段。请参考 Step 6.6 中的推荐行动，选择最匹配的下一步。
...
用户选择进入数据流设计时，调用 data-flow-model 技能：

**8.5.1 输入准备**

将需求挖掘报告作为输入传递给 data-flow-model 技能：

```text
📋 传递给 data-flow-model 的输入
━━━━━━━━━━━━━━━━
...
正在调用 data-flow-model 技能进行数据流设计...
```

**8.5.2 技能调用**

使用 `use_skill` 工具调用 `data-flow-model` 技能，传入需求挖掘报告内容。

**8.5.3 输出衔接**

data-flow-model 技能输出数据模型和数据流图后，提示用户：
...
用户选择进入验证原型时，调用 demo-verify 技能：

**8.7.1 输入准备**

将需求挖掘报告作为输入传递给 demo-verify 技能：
...
正在调用 demo-verify 技能进行验证原型...
```

**8.7.2 技能调用**

使用 `use_skill` 工具调用 `demo-verify` 技能，传入需求挖掘报告内容。

**8.7.3 输出衔接**

demo-verify 技能输出验证报告后，提示用户：
...
- **快速实现前查专家**：当复杂度评估为低复杂度、推荐直接编码实现时，必须先调用 `expert-lookup` 技能查找是否有相关的业务专家资产包。找到则加载专家资产（契约层、已知坑、实现导航等）以专家视角编码；未找到则正常编码实现
