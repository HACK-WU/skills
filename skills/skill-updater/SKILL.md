---
name: skill-updater
description: 自动更新安装脚本的技能文件列表并同步 README。当用户添加新技能、删除技能或修改技能文件结构后，使用此技能同步更新 skill-install.sh 和 skill-install.ps1 中的静态文件列表，以及 README.md 中的"技能一览"表格（"项目结构"树仅维护顶层目录，不逐技能同步）。触发短语包括："更新安装脚本"、"同步技能列表"、"新增技能后更新脚本"、"技能列表不同步"。
---
...
- `README.md` 的"技能一览"表格与实际 `skills/` 目录不一致（"项目结构"树仅维护顶层目录，不逐技能同步，不在此检查范围）
...
4. 同步更新 `README.md` 中的"技能一览"表格（"项目结构"树仅维护顶层目录，不逐技能同步）
...
安装脚本更新完成后，检查根目录 `README.md` 的"技能一览"表格是否与 `skills/` 目录一致，不一致时同步修改表格。

> **结构树约定**：`README.md` 的"项目结构"树仅维护顶层目录（`skills/`、`rules/`、`agents/`、`scripts/`、`docs/`），**不逐技能同步**。新增/删除技能时无需修改结构树，技能清单以"技能一览"表格为唯一来源。
...
| `质量与优化` | 评审、优化、专家资产、方案沉淀等质量治理能力 |
| `开发与工具` | 文档生成、技能创建、记忆管理、任务调度等开发辅助能力 |
...
删除技能时，附带检查 README 顶部"设计流程"mermaid 图中是否引用了该技能名：若有引用，提醒用户流程图需人工调整（流程图布局不适合自动修改）。
...
4. 确认 README.md 与 `skills/` 目录双向一致：
   - 正向：每个技能都出现在"技能一览"表格中（表格是技能清单的唯一来源）
   - 反向：表格中没有实际已不存在的技能（孤儿条目），表格链接路径 `./skills/<技能名>/SKILL.md` 真实存在
   - 结构树仅检查顶层目录是否齐全（`skills/`、`rules/`、`agents/`、`scripts/`、`docs/`），不检查技能级目录
...
- `read_lints` 无错误 → ✓
- README"技能一览"表格包含 `work-breakdown` 且链接真实存在 → ✓
...
7. **技能清单单一来源**：README 的技能清单以"技能一览"表格为唯一来源；"项目结构"树仅维护顶层目录，新增/删除技能只改表格，不改树
...
- `README.md` - 项目说明（"技能一览"表格为技能清单唯一来源；"项目结构"树仅维护顶层目录）
- `skills/` - 技能目录根目录
