...
description: 查询 ki-search 记忆的 SSOT：封装查 ki 的公共动作（可用性检测、scope 确定、符号提准、降级不阻塞），按记忆类型分发到专题、接口、数据流、强关联、决策、错误库、待生效变更各参考策略。触发："查记忆"、"查强关联"、"这个模块牵动哪些"、"改A要连带改B吗"、"当初为什么这么定"、"这个报错见过吗"、"ki memory lookup"。
---
