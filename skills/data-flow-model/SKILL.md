...
description: 构建数据模型（ER 图）和数据流图，明确数据结构与数据流向。支持多种场景：简单 CRUD、并发、分布式、实时流、批处理，根据场景类型叠加特定分析。适用场景：(1) "画一下 ER 图"、(2) "数据怎么流的"、(3) "帮我设计数据模型"、(4) "这个分布式场景下数据怎么同步"、(5) "并发场景下数据一致性怎么保证"、(6) 技术设计之前需要先理清数据结构和数据流向时。输入需求描述或需求挖掘报告，输出数据模型文档。
---
