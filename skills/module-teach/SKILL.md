---
name: module-teach
description: 系统讲解本项目代码：模块讲解（能力大纲 → 代码 wiki → 功能推演 → 应用场景 → 数据流）或变更讲解（commit / PR / 任意 diff：逐处解读 → 影响面 → 意图与权衡），产出可反复查阅的 Markdown 材料。触发："讲讲这个模块"、"学习一下 XXX 代码"、"讲讲这个 commit/PR"、"这个改动是什么意思"、"帮我搞懂这块代码"、"考我一下"。通用知识主题用 topic-teach。
---
...
- 渲染环境不确定（聊窗讲解 / 终端 / 纯文本传阅）或需逐节点挂 `文件:行号` 批注时，结构类信息（子模块依赖、数据流主干、状态流转链）可用 **🕸 文字树**兜底（渲染无关）；**只作兜底，不替代上述 Mermaid / SVG 默认**，最小语法与示例见 [reference.md](reference.md)「5.1 🕸 文字树」
