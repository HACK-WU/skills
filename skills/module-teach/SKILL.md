---
name: module-teach
description: 分析并讲解指定代码模块，区分通用知识与项目专用知识，产出可反复查阅的学习材料（含 Mermaid 图与 HTML 富媒体文档）。支持大模块分批讲解、按学习目标调整深度、可选知识点对齐（选择题模式，自动评分 + 跨轮次追踪进步）。当用户需要理解、接手、评审或讲解某个代码模块，或说"讲讲这个模块""学习一下XXX代码""讲解这个模块怎么工作""帮我搞懂这块代码""考我一下""知识点对齐"时使用。
---
...
- 用户说"讲讲这个模块""学习一下 XXX 代码""讲解这个模块怎么工作"时
...
- **目录初始化**：若 `.teach/{module-slug}/` 不存在（首次独立触发），创建该目录及 `quiz-history/` 子目录，无需执行 Phase 0–7。
...
- **图表配色**：所有 Mermaid / 图表 / HTML 页面背景禁用黑色或过深颜色，一律浅色背景（含白色）+ 深色文字；深色仅用于元素强调（表头 / 高亮块，其上文字用浅色）。Mermaid 保持默认浅色主题（`theme: "default"`），不启用深色主题
