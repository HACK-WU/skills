...
>
> **唯一例外**：用户显式同意下的「伴读分支」（见下节）——只在新分支上进行、只加可整体剔除的 `[teach]` 注释、不提交、不合并。
...
3. **加注释**（纪律）：只加注释、不改一行语义（不顺手格式化 / 重命名 / 修 bug）；每条教学注释**独立整行**、含 `[teach]` 标记，含文件头标识块——**删除所有含 `[teach]` 的行即还原原文件**；只覆盖教程重点文件与关键函数（Phase 2 wiki 的关键清单），不撒胡椒面；注释语言与风格随项目既有约定。模板见 [reference.md](reference.md) §9。
...
【通用 · 可选】
- [ ] Phase 8：知识点对齐 · 选择题
- [ ] 伴读分支（仅 module）：工作区干净检查 → 切 `teach/{module-slug}` → 加 `[teach]` 注释 → 自检（删除数=0）→ 记录（不提交）
```
...
> **可选追加（仅 `module` 类）**：**伴读分支**——询问用户是否把教程要点就地注释到核心代码。流程：工作区干净检查（不干净则不创建）→ `git switch -c teach/{module-slug}` → 只加 `[teach]` 注释（`git diff --numstat` 删除数必须为 0）→ 在 `06-最终文档.md` 追加「伴读分支」记录（元信息，不需重跑评审）。**AI 不代为提交。** 详见「伴读分支」小节。
