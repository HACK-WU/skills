---
name: module-teach
description: 分析并讲解指定代码模块，区分通用知识与项目专用知识，产出可反复查阅的学习材料（含 Mermaid 图与 SVG 图表）。支持大模块分批讲解、按学习目标调整深度、可选知识点对齐（选择题模式，自动评分 + 跨轮次追踪进步）。当用户需要理解、接手、评审或讲解某个代码模块，或说"讲讲这个模块""学习一下XXX代码""讲解这个模块怎么工作""帮我搞懂这块代码""考我一下""知识点对齐"时使用。
---
...
**目的**：把"读懂一个陌生代码模块"这件高认知负荷的事，结构化成一个**可校验、渐进式、带图表**的教学流程，让 AI 充当"模块讲解员"，帮助用户快速建立从全局到细节的认知。
...
├── 04-应用场景.md
├── 05-数据流.md
├── 06-最终文档.md        # 汇总全部阶段（Mermaid 代码块内嵌 + SVG 图引用）
├── assets/               # 可选：复杂图 SVG 存放于此（kebab-case 命名）
...
```
任务进度：
- [ ] Phase 0：确认范围（模块路径 / 通用 or 专用 or 两者 / 学习目标 / 深度）
...
**问题交互规则**：Phase 0 共 3 个问题，其中 **目标模块** 为必答项（用户不说则用 `list_dir` 让其选择），其余 2 项均有默认值。若用户跳过某项，直接使用默认值并在大纲中标注。
...
- **图表配色**：所有 Mermaid / SVG 图表背景禁用黑色或过深颜色，一律浅色背景（含白色）+ 深色文字；深色仅用于元素强调（表头 / 高亮块，其上文字用浅色）。Mermaid 保持默认浅色主题（`theme: "default"`），不启用深色主题

## 更多资源

- 格式规范与 Mermaid / SVG 图表规范：[reference.md](reference.md)
- 阶段产出示例与最终文档样例：[examples.md](examples.md)
