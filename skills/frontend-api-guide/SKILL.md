...
3. **可选 UI 映射**：有 UI 设计稿时，将 API 返回值与 UI 元素一一对应
4. **错误处理闭环**：每个可能的错误响应都有明确的前端行为指引
...
|---------|------|---------|---------|--------|
| 用户信息卡 | 头像 | `data.avatar` | 直接显示 | 默认头像 |
| 用户信息卡 | 姓名 | `data.name` | 直接显示 | "未设置" |
| 用户信息卡 | 等级 | `data.level` | 枚举映射：1→普通 2→VIP | "普通" |
| 订单列表 | 总金额 | `data.orders[].total` | 格式化：¥{amount} | - |
```
