...
- **优化建议两条路、每项可排序**：技术优化（功能等价，不改产品行为）+ 产品侧调整（交 `product-manager`：**退让型**牺牲部分功能换性能；**改进型**不牺牲功能，找原设计缺陷给更优设计，体验与性能双升）；每项给**推荐指数（★1-5）**与**预期效果**
...
- **呈现口径**：每项在报告中先用**一句话**说清并给**推荐指数 + 预期效果**（技术优化：改什么 → 省掉什么；产品侧：让出什么 / 改进什么 → 换多少收益），细节进表；**优化建议一律不配图**；技术优化项与阶段 2.4 / [reference.md](reference.md) 7.3 的 Δ 分解口径对上（`ΔN×t` / `N×Δt`）；**报告正文不写内部用语（"A / B 路径、双路径、B1 / B2"等）**
...
## 五、优化建议
五·一 技术优化（不改功能，本章不配图）：每项 推荐指数 + 一句话（改什么 → 省掉什么）+ 预期效果 + 表
...
报告可内嵌 mermaid 图，但**图的使命只有一个：卡在哪**——📊 分段耗时（时间卡在哪段）+ 🔗 链路拓扑（卡在哪个节点）+ 🕐 卡点时序（阻塞 / 并行结构）+ 📅 **gantt 关键路径**（秒级及以上、串并行多段时首选：一眼看出谁在等谁、哪段被掩盖）+ 📊 双样本差异（可选：🔍 归因链）。**硬禁止两类图**：① **决策路径类**（是否达标 / A·B 取舍 / 审批 / 排期）——一律用表或文字，禁止任何形式的 mermaid 图；② **优化建议类**（机理图 / 前后对比图）——优化项用一句话说清，"省掉什么"写进句子。**铁律**：① 图 / 一句话 / 表三者数字一致，图与一句话都不是第二份结论；② 🔴 假设画成虚线 / 空心并标"待验证"，否则图会把假设视觉坐实；③ 说不出"省掉什么"的优化项 = 未论证，不得进结论区；④ 不堆图（⚡ 0-1 / 📊 2 / 🧑‍💻 3 张）。模板与语法安全见 [reference.md](reference.md)「九、报告图表规范」。
...
- 八层详细检查清单、技术栈速查（Python/Java/Go/Node/DB/Redis/网关）、收益估算示例、证据采集命令速查、三档位完整报告模板、**双样本差分分析（含变量对齐模板与 ΔN/Δt 算式）**、**优化建议双路径（推荐指数与预期效果口径 / 退让点候选清单 / 改进型设计缺陷识别 / 交 product-manager 的输入包与回执字段 / 九种退让形态 / 收益叠加口径）**、**报告图表规范（分段耗时 / 拓扑 / 时序 / gantt 关键路径模板、一句话优化建议写法、目录锚点规则、语法安全）**、**交付前自查清单（三遍法 / 14 项核验 / 未通过处理）**：[reference.md](reference.md)
