...
| 规则 | 例 |
|------|----|
| 中文、字母原样保留，字母转小写 | `五·B、功能换性能` → `#五b功能换性能` |
| 标点与空格全部去掉（`、` `·` `：` `（）`） | `四、主瓶颈与归因` → `#四主瓶颈与归因` |
| **标题不放 emoji**（emoji 会让锚点不可预测） | `## 六、待验证清单（🔴 假设）` → 写成 `## 六、待验证清单` |
| **标题行不放说明文字**（否则会拼进锚点） | `## 五、优化建议（双路径）　← 本章不配图` → 标题只写 `## 五、优化建议（双路径）`，说明另起一行 |
...
| 优先级 | 优化项（一句话：改什么 → 省掉什么 → 收益上限） | 对应瓶颈 | 功能等价性说明 | 副作用与风险 | 落地成本 |
|--------|------------------------------------------------|----------|----------------|--------------|----------|
| P0 | 明细查询由循环单条改批量，省掉约 800 次往返，明细段 1200 → 400ms，端到端上限约 -25% | B-1 | 返回集合与顺序一致 | 低 | 低 |
...
> 每项同样以**一句话**为主呈现：`让出 <能力> → 换 <收益上限>`（明细见本表）。B 路径项的决策状态由 `product-manager` 给出；未做产品分析时一律标「⚠️ 待产品确认」，并在表下注明"本表未经产品分析，不作为决策依据"。B 项在拍板前不进 P0 落地计划。
...
- 五·A 表在「一句话」列右侧追加「改法细节」列：具体到改哪个文件、哪个参数、改成什么（一句话仍保留，读者先看结论）
...
**呈现口径（A / B 通用）**：报告中每项先用**一句话**说清——A 项 `改什么 → 省掉什么 → 收益上限`，B 项 `让出什么 → 换多少收益`；细节进表。**优化建议一律不配图**：机理图、前后对比图、决策流程图都不画——"省掉什么"（往返次数 / 等待时长 / 重复计算量）写进句子即可，图只会让读者更累。
...
| 🔗 链路拓扑图 | 卡在哪个节点（N+1 / 全表扫描 / 连接池等待） | 开发 / 双读者档 |
| 🕐 卡点时序图 | 卡在哪个调用、等了多久（阻塞 / 串行 / 并行结构） | 调用形态是主因时必带 |
| 📊 双样本对比图 | 慢样本与快样本差在哪一段 | 命中双样本 |
| 🔍 归因链图 | 为什么会卡（现象 → 证据 → 根因 → 影响） | 可选，文字能说清就不画 |
...
> **张数上限**：⚡ 0-1 / 📊 2 / 🧑‍💻 3 张。超出上限时按优先级保留——**分段耗时 > 卡点时序 / 拓扑 > 双样本对比**，其余降级为表格一句话。
...
| 一句话与表数字不一致 | 正文句子写 -25%、表里写 -37%，两个版本流入评审 | 6.2 铁律①：图 / 一句话 / 表三者同源，落盘前逐项核对 |
