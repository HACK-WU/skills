...
> **张数上限**：⚡ 0-1 / 📊 2 / 🧑‍💻 3 张。超出上限时按优先级保留——**分段耗时 > gantt / 卡点时序 / 拓扑 > 双样本对比**，其余降级为表格一句话。
...
| 4 | 数字三方一致 | 报告内互核：执行摘要 ↔ 耗时表 ↔ 图 ↔ 一句话 ↔ 收益表；再抽 3-5 个关键数字**回对原始证据**（压测 / trace / 慢日志） | 改表忘改图 / 一句话；转录错数字（38357 写成 38573） |
...
| 7 | 图表合规 | 只画"卡在哪"（分段耗时 / 拓扑 / 时序 / gantt / 双样本）；无决策图与优化建议图；张数 ≤ 档位上限；每图有表 | 多画一张机理图、堆图 |
