...
将 Step 7 的报告**提炼为结论性摘要**后写入文件，添加元数据头。

持久化的是结论，不是过程。下游 skill 需要快速引用关键信息，而非阅读完整分析过程。
...
首次配置后，将存储位置写入 `.requirements/config`：

```markdown
# Requirements Config

storage_path: {用户选择的路径}
```
