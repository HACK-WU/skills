...
> `--from`/`--to` 校验格式 `YYYY-MM-DD` 或 `YYYY-MM-DD HH:MM:SS`，并验证日历合法性（如 `2026-13-99`、`25:61:00` 会拒绝），非法报错退出 1；`--from > --to`（区间为空）报错退出 1。`--to` 为纯日期时自动补为当天末尾 `23:59:59`（含当日全部记录）。
...
- 失败：stdout 输出 `{"ok": false, "error": "..."}`（错误信息由 stderr 文本提炼：剥离 `错误:` 前缀，并过滤 `⚠/警告/ℹ/提示` 开头的非阻断提示行，`error` 只含真正的错误）。
- 未预期异常（非业务报错的程序崩溃）同样输出 `{"ok": false, "error": "..."}` 并退出 1，调用方永远能拿到可解析的 JSON。
