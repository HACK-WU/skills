...
快速通道跳过阶段 2.5（拆分）和阶段 3（多文档大纲），直接用精简 14 章模板输出。
...
4. **一览图必须**：拆分后必须输出关键环节一览图，将子需求作为节点展示全局流程
5. **分阶段确认**：每个关键阶段必须等用户确认才能继续
6. **不脑补需求**：发现需求模糊或场景不成立时，停下来，而非自行补全
...
| 成熟度 | 处理方式 |
|--------|---------|
| 高 | 输出「已有理解摘要」一次性确认，跳过阶段 1 和阶段 2，直接进入阶段 2.5 |
