...
| **[performance-analysis](./skills/performance-analysis/SKILL.md)** | 接口/请求链路性能专项分析：盘点现有证据（火焰图/APM/压测/慢查询）与代码，八层定位瓶颈；支持**双样本差分**（一快一慢两份数据，先变量对齐再 Δ 分解），量化收益上限与推荐指数；性能不佳时输出两条路建议（技术优化 + 产品侧调整：**退让型**牺牲部分功能换性能 / **改进型**修原设计缺陷实现体验与性能双升，均交 product-manager 分析），产出带目录索引、只画「卡在哪」的图（含 **gantt 关键路径**；决策路径与优化建议不画图）、每条建议带**推荐指数与预期效果**的面向开发/产品的报告 + 验证计划 | "性能分析"、"为什么这么慢"、"接口 RT 高"、"性能优化"、"出份性能报告"、"容量评估"、"两份数据对比"、"优化前后对比" |
