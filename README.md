...
| **[topic-teach](./skills/topic-teach/SKILL.md)** | 教学通用知识主题（k8s/docker/Python 等技术与投资/理财等非技术领域），产出含类比、Mermaid 图与 SVG（含数据流全链路图）的学习材料，支持课程制/速览双模式、大纲教学效果推演、环境准备课、课级应用实战（按需配套、第四幕渐进演进）、多角色主题拆子教程、结课综合实战项目、实战经验与排障速查手册、场景解法库（设计题独立成册：多解法 + 代码 + 替代路线 + 按需设计图） | "教我k8s"、"讲讲Python装饰器"、"什么是ETF"、"给我个项目练手" |
